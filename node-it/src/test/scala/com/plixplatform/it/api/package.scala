...
import com.plixlatform.transaction.assets.exchange.AssetPair
...
package object api {
  ...
  implicit val addressReads: Reads[com.plixlatform.account.Address] = Reads {
  ...
  implicit val dstMapReads: Reads[Map[com.plixlatform.account.Address, Long]] = Reads { json =>
    json.validate[Map[String, Long]].map { dst =>
      dst.map {
        case (addrStr, balance) =>
          com.plixlatform.account.Address.fromString(addrStr).explicitGet() -> balance
      }
    }
  }
  ...
  implicit val distributionReads: Reads[AssetDistribution] = Reads { json =>
    json
      .validate[Map[com.plixlatform.account.Address, Long]]
  ...
  implicit val distributionPageReads: Reads[AssetDistributionPage] = Reads { json =>
    json.validate[Paged[com.plixlatform.account.Address, AssetDistribution]].map(pg => AssetDistributionPage(pg))
  }
}
