...
import com.plixlatform.account.{AddressOrAlias, AddressScheme, KeyPair}
import com.plixlatform.api.http.assets.{SignedIssueV1Request, SignedIssueV2Request}
import com.plixlatform.api.http.AddressApiRoute
import com.plixlatform.common.state.ByteStr
import com.plixlatform.common.utils.EitherExt2
import com.plixlatform.features.api.{ActivationStatus, FeatureActivationStatus}
import com.plixlatform.http.DebugMessage
...
import com.plixlatform.transaction.assets.IssueTransactionV2
import com.plixlatform.transaction.lease.{LeaseCancelTransactionV2, LeaseTransactionV2}
import com.plixlatform.transaction.smart.InvokeScriptTransaction
...
object SyncHttpApi extends Assertions {
  ...
  implicit class NodeExtSync(n: Node) extends Assertions with Matchers {
    import com.plixlatform.it.api.AsyncHttpApi.{NodeAsyncHttpApi => async}
  ...
  implicit class NodesExtSync(nodes: Seq[Node]) {
    ...
    import com.plixlatform.it.api.AsyncHttpApi.{NodesAsyncHttpApi => async}
    ...
    def rollback(height: Int, returnToUTX: Boolean = true): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.plixlatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollback(height, returnToUTX)
        },
        ConditionAwaitTime
      )
    }
    ...
    def waitForHeight(height: Int): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.plixlatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).waitForHeight(height)
        },
        ConditionAwaitTime
      )
    }
