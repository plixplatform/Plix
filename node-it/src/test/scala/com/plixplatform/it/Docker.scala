...
import com.plixlatform.it.api.AsyncHttpApi._
import com.plixlatform.it.util.GlobalTimer.{instance => timer}
...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false, imageName: String = Docker.NodeImageName)
  ...
  private val logDir: Coeval[Path] = Coeval.evalOnce {
    val r = Option(System.getProperty("plix.it.logging.dir"))
      .map(Paths.get(_))
  ...
  private lazy val plixNetwork: Network = {
    val id          = Random.nextInt(Int.MaxValue)
    val networkName = s"plix-$id"
  ...
  def createNetwork: Network = plixNetwork
  ...
  private def startNodeInternal(nodeConfig: Config): DockerNode =
    try {
      ...
      val networkPort = actualConfig.getString("plix.network.port")
      val hostConfig = HostConfig
        .builder()
        .publishAllPorts(true)
        .build()

      val nodeName   = actualConfig.getString("plix.network.node-name")
      ...
      val configOverrides: String = {
        val ntpServer    = Option(System.getenv("NTP_SERVER")).fold("")(x => s"-Dplix.ntp-server=$x ")
        val maxCacheSize = Option(System.getenv("MAX_CACHE_SIZE")).fold("")(x => s"-Dplix.max-cache-size=$x ")

        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dplix.network.declared-address=$ip:$networkPort $ntpServer $maxCacheSize"
        ...
        if (enableProfiling) {
          ...
          config += s"-agentpath:/usr/local/YourKit-JavaProfiler-2019.1/bin/linux-x86-64/libyjpagent.so=port=$ProfilerPort,listen=all," +
            s"sampling,monitors,sessionname=PlixNode,dir=$ContainerRoot/profiler,logdir=$ContainerRoot,onexit=snapshot "
        }
      ...
      val containerConfig = ContainerConfig
        .builder()
        .image(imageName)
        .networkingConfig(ContainerConfig.NetworkingConfig.create(Map(plixNetwork.name() -> endpointConfigFor(nodeName)).asJava))
        .hostConfig(hostConfig)
        .env(s"PLIX_OPTS=$configOverrides")
        .build()

      val containerId = {
        val containerName = s"${plixNetwork.name()}-$nodeName"
        dumpContainers(
      ...
      val node = new DockerNode(actualConfig, containerId, getNodeInfo(containerId, PlixSettings.fromRootConfig(actualConfig)))
      nodes.add(node)
  ...
  private def getNodeInfo(containerId: String, settings: PlixSettings): NodeInfo = {
    ...
    val plixIpAddress = containerInfo.networkSettings().networks().get(plixNetwork.name()).ipAddress()

    NodeInfo(restApiPort, networkPort, plixIpAddress, containerInfo.networkSettings().ports(), dockerIpAddress)
  }

  private def inspectContainer(containerId: String): ContainerInfo = {
    val containerInfo = client.inspectContainer(containerId)
    if (containerInfo.networkSettings().networks().asScala.contains(plixNetwork.name())) containerInfo
    else {
      log.debug(s"Container $containerId has not connected to the network ${plixNetwork.name()} yet, retry")
      Thread.sleep(1000)
  ...
  def restartNode(node: DockerNode, configUpdates: Config = empty): DockerNode = {
    ...
    if (configUpdates != empty) {
      ...
      val shPath = "/opt/plix/start-plix.sh"
      val scriptCmd: Array[String] =
        Array("sh", "-c", s"sed -i 's|$$PLIX_OPTS.*-cp|$$PLIX_OPTS $renderedConfig -cp|' $shPath && chmod +x $shPath")
  ...
  override def close(): Unit = {
    if (isStopped.compareAndSet(false, true)) {
      ...
      try {
        client.removeNetwork(plixNetwork.id)
      } catch {
        case NonFatal(e) =>
          // https://github.com/moby/moby/issues/17217
          log.warn(s"Can not remove network ${plixNetwork.name()}", e)
      }
  ...
  private def disconnectFromNetwork(containerId: String): Unit = {
    ...
    client.disconnectFromNetwork(containerId, plixNetwork.id())
  }
...
object Docker {
  val NodeImageName: String = "com.plixlatform/node-it:latest"

  private val ContainerRoot = Paths.get("/opt/plix")
  ...
  def genesisOverride: Config = {
    ...
    val timestampOverrides = parseString(s"""plix.blockchain.custom.genesis {
                                            |  timestamp = $genesisTs
    ...
    val gs               = genesisConfig.as[GenesisSettings]("plix.blockchain.custom.genesis")
    val genesisSignature = Block.genesis(gs).explicitGet().uniqueId

    parseString(s"plix.blockchain.custom.genesis.signature = $genesisSignature").withFallback(timestampOverrides)
  }

  AddressScheme.current = new AddressScheme {
    override val chainId: Byte = configTemplate.as[String]("plix.blockchain.custom.address-scheme-character").charAt(0).toByte
  }
  ...
  case class NodeInfo(restApiPort: Int, networkPort: Int, plixIpAddress: String, ports: JMap[String, JList[PortBinding]], dockerIpAddress: String = "localhost") {
    ...
    val containerNetworkAddress: InetSocketAddress = new InetSocketAddress(plixIpAddress, networkPort)
