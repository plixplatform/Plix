...
class IssueNFTSuite extends BaseTransactionSuite with TableDrivenPropertyChecks {
  ...
  test("Can't issue NFT before activation") {
    ...
    assertBadRequest(
      firstNode.issue(firstAddress, assetName, assetDescription, 1, 0, reissuable = false, 1.plix / 1000, waitForTx = true)
    )
  }
  ...
  test("nft assets balance should be returned by separate api endpoint") {
    secondNode
      .issue(secondNode.address, "Common", "Common asset", quantity = 1, decimals = 1, reissuable = false, fee = 1.plix, script = None)
      .id
