...
class TransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("asset transfer changes sender's and recipient's asset balance; issuer's.plix balance is decreased by fee") {
  ...
  test("plix transfer changes plix balances and eff.b.") {
    for (v <- supportedVersions) {
      val (firstBalance, firstEffBalance)   = miner.accountBalances(firstAddress)
      val (secondBalance, secondEffBalance) = miner.accountBalances(secondAddress)
  ...
  test("invalid signed plix transfer should not be in UTX or blockchain") {
    def invalidTx(timestamp: Long = System.currentTimeMillis, fee: Long = 100000): TransferTransactionV1.TransactionT =
      TransferTransactionV1
        .selfSigned(Plix, sender.privateKey, AddressOrAlias.fromString(sender.address).explicitGet(), 1, timestamp, Plix, fee, Array.emptyByteArray)
        .right
        .get
  ...
  test("can not make transfer without having enough balance") {
    for (v <- supportedVersions) {
      ...
      assertBadRequestAndResponse(sender.transfer(secondAddress, firstAddress, secondBalance + 1.plix, minFee, version = v),
