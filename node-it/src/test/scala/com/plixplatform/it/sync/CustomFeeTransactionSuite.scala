...
import com.plixlatform.state.Sponsorship
...
class CustomFeeTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("make transfer with sponsored asset") {
    ...
    val sponsorship = Sponsorship.toPlix(transferFee, assetToken)
...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""
                                                         | plix.fees.transfer.$assetId = 100000
                                                         | plix.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("plix.miner.enable=no").withFallback(minerConfig)
