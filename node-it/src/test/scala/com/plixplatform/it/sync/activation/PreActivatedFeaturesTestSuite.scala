package com.plixplatform.it.sync.activation
import com.typesafe.config.{Config, ConfigFactory}
import com.plixplatform.features.api.NodeFeatureStatus
import com.plixplatform.features.{BlockchainFeatureStatus, BlockchainFeatures}
import com.plixplatform.it.{Docker, ReportingTestName}
...
object PreActivatedFeaturesTestSuite {
  ...
  private val supportedConfig    = ConfigFactory.parseString(s"""plix {
                                                             ...
                                                             |  miner.quorum = 1
                                                             |}""".stripMargin)
  private val preactivatedConfig = ConfigFactory.parseString(s"""plix {
                                                                |  blockchain.custom.functionality {
                                                                |  feature-check-blocks-period = $votingInterval
                                                                |  pre-activated-features {
                                                                |        1: 0
