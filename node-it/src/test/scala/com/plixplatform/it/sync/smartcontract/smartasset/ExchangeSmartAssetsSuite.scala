...
import com.plixlatform.it.sync.smartcontract.{cryptoContextScript, pureContextScript, plixContextScript, _}
...
class ExchangeSmartAssetsSuite extends BaseTransactionSuite with CancelAfterFailure with NTPTime {
  ...
  test("require using a certain matcher with smart accounts") {
    ...
    val smartPair = AssetPair(IssuedAsset(ByteStr.decodeBase58(sAsset).get), Plix)
  ...
  test("AssetPair from smart assets") {
    ...
    withClue("check fee for smart accounts and smart AssetPair - extx.fee == 0.015.plix") {
      ...
      assertBadRequestAndMessage(
        ...
        "does not exceed minimal value of 1500000 PLIX"
      )
  ...
  test("use all functions from RIDE for asset script") {
    ...
    val script3 = Some(ScriptCompiler(plixContextScript(dtx, false), isAssetScript = true).explicitGet()._1.bytes.value.base64)
    ...
    List(script1, script2, script3)
      .map { i =>
        ...
        val smartPair = AssetPair(IssuedAsset(ByteStr.decodeBase58(asset).get), Plix)
