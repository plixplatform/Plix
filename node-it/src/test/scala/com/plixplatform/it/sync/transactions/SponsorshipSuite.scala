...
import com.plixplatform.it.{NodeConfigs, ReportingTestName}
import com.plixplatform.state.diffs.FeeValidation
import com.plixplatform.transaction.Asset.IssuedAsset
import com.plixplatform.transaction.assets.SponsorFeeTransaction
...
class SponsorshipSuite extends FreeSpec with NodesFromDocker with Matchers with ReportingTestName with CancelAfterFailure {
  ...
  override def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("plix.blockchain.custom.functionality.blocks-for-feature-activation=1"))
      .overrideBase(_.raw("plix.blockchain.custom.functionality.feature-check-blocks-period=1"))
      .withDefault(1)
  ...
  "Fee in sponsored asset works fine" - {
    ...
    val sponsorPlixBalance = sponsor.accountBalances(sponsor.address)._2
    val minerPlixBalance   = miner.accountBalances(miner.address)._2
    ...
    "check balance before test accounts balances" in {
      ...
      sponsor.assertBalances(sponsor.address, sponsorPlixBalance - 2 * issueFee - minFee)
    ...
    val minerPlixBalanceAfterFirstXferTest   = minerPlixBalance + 2.plix + minFee + FeeValidation.FeeUnit * SmallFee / minSponsorFee
    val sponsorPlixBalanceAfterFirstXferTest = sponsorPlixBalance - 2.plix - minFee - FeeValidation.FeeUnit * SmallFee / minSponsorFee
...
//        sponsorTx.head.count(tx => tx.sender.contains(sponsor.address) || tx.recipient.contains(sponsor.address)) shouldBe 4
      ...
      "sponsor should receive sponsored asset as fee, plix should be written off" in {
        miner.assertAssetBalance(sponsor.address, sponsorAssetId, sponsorAssetTotal / 2 + SmallFee)
        miner.assertBalances(sponsor.address, sponsorPlixBalanceAfterFirstXferTest)
      }

      "miner plix balance should be changed" in {
        miner.assertBalances(miner.address, minerPlixBalanceAfterFirstXferTest)
      }
    }

    "assets balance should contain sponsor fee info and sponsor balance" in {
      val sponsorLeaseSomePlix = sponsor.lease(sponsor.address, bob.address, leasingAmount, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseSomePlix)
    ...
    "plix fee depends on sponsor fee and sponsored token decimals" in {
      val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.plix, LargeFee, None, Some(sponsorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
      assert(!transferTxCustomFeeAlice.isEmpty)
      ...
      miner.assertBalances(
        sponsor.address,
        sponsorPlixBalanceAfterFirstXferTest - FeeValidation.FeeUnit * LargeFee / Token - leasingFee,
        sponsorPlixBalanceAfterFirstXferTest - FeeValidation.FeeUnit * LargeFee / Token - leasingFee - leasingAmount
      )
      miner.assertBalances(miner.address, minerPlixBalanceAfterFirstXferTest + FeeValidation.FeeUnit * LargeFee / Token + leasingFee)
    }
    ...
    "cancel sponsorship" - {
      ...
      "check sponsor and miner balances after cancel" in {
        miner.assertBalances(
          sponsor.address,
          sponsorPlixBalanceAfterFirstXferTest - FeeValidation.FeeUnit * LargeFee / Token - leasingFee - issueFee,
          sponsorPlixBalanceAfterFirstXferTest - FeeValidation.FeeUnit * LargeFee / Token - leasingFee - leasingAmount - issueFee
        )
        miner.assertBalances(miner.address, minerPlixBalanceAfterFirstXferTest + FeeValidation.FeeUnit * LargeFee / Token + leasingFee + issueFee)
      }
    ...
    "set sponsopship again" - {
      ...
      "make transfer with new min sponsored fee" in {
        ...
        val alicePlixBalance   = alice.accountBalances(alice.address)
        val bobAssetBalance     = bob.assetBalance(bob.address, sponsorAssetId).balance
        val bobPlixBalance     = bob.accountBalances(bob.address)
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.plix, TinyFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        nodes.waitForHeightArise()

        val plixFee = FeeValidation.FeeUnit * TinyFee / TinyFee
        sponsor.assertBalances(sponsor.address, sponsoredBalance._1 - plixFee, sponsoredBalance._2 - plixFee)
        ...
        alice.assertBalances(alice.address, alicePlixBalance._2 - 1.plix)
        bob.assertBalances(bob.address, bobPlixBalance._1 + 1.plix, bobPlixBalance._2 + 1.plix)
        bob.assertAssetBalance(bob.address, sponsorAssetId, bobAssetBalance)
        miner.assertBalances(miner.address, minerBalance._2 + plixFee)
      ...
      "transfer tx sponsored fee is less then new minimal" in {
        assertBadRequestAndResponse(
          sponsor
            ...
            .id,
          s"Fee for TransferTransaction \\($SmallFee in ${Some(sponsorAssetId).get}\\) does not exceed minimal value of 100000 PLIX or $LargeFee ${Some(sponsorAssetId).get}"
        )
      }
      ...
      "make transfer with updated min sponsored fee" in {
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.plix, LargeFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        val plixFee = FeeValidation.FeeUnit * LargeFee / LargeFee
        nodes.waitForHeightArise()
    ...
    "issue asset make sponsor and burn and reissue" in {
      ...
      val aliceTransferPlix = alice.transfer(alice.address, bob.address, transferAmount, SmallFee, None, Some(sponsorAssetId2)).id
      nodes.waitForHeightAriseAndTxPresent(aliceTransferPlix)
      nodes.waitForHeightArise()

      val totalPlixFee = FeeValidation.FeeUnit * SmallFee / Token + issueFee + sponsorFee + burnFee + minFee + issueFee
      miner.assertBalances(miner.address, minerBalance._1 + totalPlixFee)
      sponsor.assertBalances(sponsor.address, sponsorBalance._1 - totalPlixFee, sponsorBalance._2 - totalPlixFee)
    ...
    "miner is sponsor" in {
      ...
      val aliceSponsoredTransferPlix = alice.transfer(alice.address, bob.address, transferAmount, SmallFee, None, Some(minersSpondorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(aliceSponsoredTransferPlix)
      nodes.waitForHeightArise()
    ...
    "tx is declined if sponsor has not enough effective balance to pay fee" in {
      ...
      val sponsorLeaseAllAvaliablePlix             = sponsor.lease(sponsor.address, bob.address, sponsorEffectiveBalance - leasingFee, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseAllAvaliablePlix)
      assertBadRequestAndMessage(alice.transfer(alice.address, bob.address, 10 * Token, LargeFee, Some(sponsorAssetId), Some(sponsorAssetId)),
                                 "unavailable funds")
      val cancelLeasingTx = sponsor.cancelLease(sponsor.address, sponsorLeaseAllAvaliablePlix, leasingFee).id
