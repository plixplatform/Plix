...
import com.plixplatform.it.api.{DataResponse, DebugStateChanges, StateChangesDetails, TransactionInfo, TransfersInfoResponse}
import com.plixplatform.it.api.SyncHttpApi._
import com.plixplatform.it.sync.setScriptFee
import com.plixplatform.it.transactions.BaseTransactionSuite
import com.plixplatform.it.util._
import com.plixplatform.lang.v1.compiler.Terms.{CONST_LONG, CONST_STRING}
...
class InvokeScriptTransactionStateChangesSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("prepare") {
    ...
    sender.massTransfer(contract.address, List(Transfer(caller.address, 3000), Transfer(recipient.address, 3000)), 0.01.plix, Some(simpleAsset))
    sender.massTransfer(contract.address, List(Transfer(caller.address, 3000), Transfer(recipient.address, 3000)), 0.01.plix, Some(assetSponsoredByDApp))
    sender.massTransfer(recipient.address, List(Transfer(caller.address, 3000), Transfer(contract.address, 3000)), 0.01.plix, Some(assetSponsoredByRecipient))
    ...
    val script = ScriptCompiler.compile(
      """
        ...
        |
        |@Callable(i)
        |func writeAndSendPlix(value: Int, recipient: String, amount: Int) = {
        |    ScriptResult(
        |        WriteSet([DataEntry("result", value)]),
