...
import com.plixlatform.it.sync.smartcontract.exchangeTx
...
import com.plixlatform.transaction.assets.IssueTransactionV1
...
class ExchangeTransactionSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("cannot exchange non-issued assets") {
    for ((o1ver, o2ver, tver) <- versions) {
      ...
      val buyAmount           = 1
      val sellAmount          = 1
      pair = AssetPair.createAssetPair("PLIX", assetId).get
  ...
  test("negative - check orders v2 and v3 with exchange tx v1") {
    ...
    pair = AssetPair.createAssetPair("PLIX", exchAsset.id().base58).get
  ...
  test("exchange tx with orders v3") {
    ...
    for ((o1ver, o2ver, matcherFeeOrder1, matcherFeeOrder2) <- Seq(
           (1: Byte, 3: Byte, Plix, IssuedAsset(assetId)),
           (1: Byte, 3: Byte, Plix, Plix),
           (2: Byte, 3: Byte, Plix, IssuedAsset(assetId)),
           (3: Byte, 1: Byte, IssuedAsset(assetId), Plix),
           (2: Byte, 3: Byte, Plix, Plix),
           (3: Byte, 2: Byte, IssuedAsset(assetId), Plix)
         )) {
      ...
      if (matcherFeeOrder1 == Plix && matcherFeeOrder2 != Plix) {
      ...
      val assetPair  = AssetPair.createAssetPair("PLIX", assetId.base58).get
