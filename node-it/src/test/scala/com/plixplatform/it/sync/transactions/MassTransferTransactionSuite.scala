...
import com.plixplatform.api.http.assets.{MassTransferRequest, SignedMassTransferRequest}
...
import com.plixplatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
import com.plixplatform.transaction.transfer.TransferTransaction.MaxAttachmentSize
...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("asset mass transfer changes asset balances and sender's.plix balance is decreased by fee.") {
  ...
  test("plix mass transfer changes plix balances") {
    val (balance1, eff1) = miner.accountBalances(firstAddress)
    val (balance2, eff2) = miner.accountBalances(secondAddress)
  ...
  test("can not make mass transfer without having enough plix") {
    val (balance1, eff1) = miner.accountBalances(firstAddress)
    val (balance2, eff2) = miner.accountBalances(secondAddress)
    val transfers        = List(Transfer(secondAddress, balance1 / 2), Transfer(thirdAddress, balance1 / 2))

    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative plix balance")
  ...
  test("invalid transfer should not be in UTX or blockchain") {
    ...
    def request(transfers: List[Transfer] = List(Transfer(secondAddress, transferAmount)),
      ...
      val txEi = for {
        ...
        tx              <- MassTransferTransaction.selfSigned(Plix, sender.privateKey, parsedTransfers, timestamp, fee, attachment)
      } yield tx
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.plix), Transfer(secondAddress, 2.plix), Transfer(thirdAddress, 3.plix))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.plix)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.plix)
  ...
  test("reporting MassTransfer transactions to aliases") {
    ...
    val transfers = aliases.map { alias =>
      Transfer(Alias.create(alias).explicitGet().stringRepr, 2.plix)
    }
    val txId = sender.massTransfer(firstAddress, transfers, 300000).id
    nodes.waitForHeightAriseAndTxPresent(txId)
