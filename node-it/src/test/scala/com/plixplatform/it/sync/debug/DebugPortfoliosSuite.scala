...
import com.plixplatform.it.{Node, NodeConfigs}
...
class DebugPortfoliosSuite extends FunSuite with NodesFromDocker {
  ...
  override protected def beforeAll(): Unit = {
    super.beforeAll()
    sender.transfer(sender.address, firstAddress, 20.plix, minFee, waitForTx = true)
    sender.transfer(sender.address, secondAddress, 20.plix, minFee, waitForTx = true)
  }
  ...
  test("getting a balance considering pessimistic transactions from UTX pool - changed after UTX") {
    ...
    sender.transfer(firstAddress, secondAddress, 5.plix, 5.plix)
    sender.transfer(secondAddress, firstAddress, 7.plix, 5.plix)

    sender.waitForUtxIncreased(utxSizeBefore)

    val portfolioAfter = sender.debugPortfoliosFor(firstAddress, considerUnspent = true)

    val expectedBalance = portfolioBefore.balance - 10.plix // withdraw + fee
  ...
  test("getting a balance without pessimistic transactions from UTX pool - not changed after UTX") {
    ...
    sender.transfer(firstAddress, secondAddress, 5.plix, fee = 5.plix)
    sender.waitForUtxIncreased(utxSizeBefore)
