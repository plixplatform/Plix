...
import com.plixlatform.it.sync.{issueFee, minFee, smartFee, smartMinFee}
...
class InvokeScriptWithSmartAccountAndAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_send plix to dApp and caller accounts") {
    val dAppTransferId        = sender.transfer(sender.address, dApp.address, 5.plix, minFee).id
    val callerTransferId      = sender.transfer(sender.address, caller.address, 5.plix, minFee).id
    val smartCallerTransferId = sender.transfer(sender.address, smartCaller.address, 5.plix, minFee).id
  ...
  test("invoke by smart account requires just 1 extra fee") {
    assertBadRequestAndMessage(
      ...
      s"does not exceed minimal value of 900000 PLIX"
    )
  }

  test("max fee is 0.053 Plix (0.005 + extraFee(1 smart caller + 1 payment + 10 transfers))") {
  ...
  test("can't invoke with small fee for both smart assets") {
    ...
    assertBadRequestAndMessage(
      sender
        ...
        .id,
      "does not exceed minimal value of 900000 PLIX"
    )
  }
