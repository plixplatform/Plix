...
import com.plixplatform.it.api._
...
import com.plixplatform.transaction.assets.{IssueTransaction, IssueTransactionV1}
...
class NFTBalanceSuite
  ...
  protected val theNodes: Coeval[Seq[Node]] = Coeval.evalOnce {
    Option(System.getProperty("plix.it.config.file")) match {
  ...
  protected override def beforeAll(): Unit = {
    ...
    val fundAndIssue =
      for {
        ...
        fundTx <- node.transfer(node.address, issuer.address, 1000.plix, 0.001.plix)
  ...
  val configs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw(s"""
                          |plix.blockchain.custom.functionality.pre-activated-features = {
                          |          2 = 0
                          |          3 = 0
                          |          4 = 0
                          |          5 = 0
    ...
    val transactions =
      Future.sequence(addrs map { addr =>
        NodeAsyncHttpApi(faucet)
          .transfer(faucet.address, addr, 1000.plix, 0.001.plix)
          .flatMap { tx =>
            NodeAsyncHttpApi(faucet)
