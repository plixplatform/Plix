...
import com.plixlatform.transaction.assets.exchange.{AssetPair, ExchangeTransactionV2, Order}
...
package object smartcontract {
  ...
  def plixContextScript(dtx: DataTransaction, accountScript: Boolean): String =
    s"""
       | match tx {
       |  case ext : ExchangeTransaction =>
       |    # Plix context
       ...
       |
       |     let balances = assetBalance(ext.sender, unit) > 0 && plixBalance(ext.sender) != 0
       |
