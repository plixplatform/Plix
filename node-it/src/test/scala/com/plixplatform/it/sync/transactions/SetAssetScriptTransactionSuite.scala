...
import com.plixplatform.it.sync.{script, someAssetAmount, _}
...
import com.plixplatform.transaction.assets.SetAssetScriptTransaction
...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("cannot transact without having enough plix") {
    val (balance, eff) = miner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.setAssetScript(assetWScript, firstAddress, balance + 1, Some(scriptBase64)), "negative plix balance")
