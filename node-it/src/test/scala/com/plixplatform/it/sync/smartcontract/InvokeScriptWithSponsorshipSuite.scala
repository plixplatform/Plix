...
import com.plixlatform.it.sync.{minFee, smartMinFee}
...
class InvokeScriptWithSponsorshipSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_send plix to dApp and caller accounts") {
    sender.transfer(sender.address, dApp.address, 5.plix, minFee, waitForTx = true).id
    sender.transfer(sender.address, caller.address, 5.plix, minFee, waitForTx = true).id
  }
  ...
  test("required fee in sponsored assets considers scripts count") {
    ...
    assertBadRequestAndMessage(
      ...
      s"does not exceed minimal value of 900000 PLIX or $feeAmount"
    )
    assertBadRequestAndMessage(
      sender.invokeScript(
        caller.address,
      ...
      s"does not exceed minimal value of 5300000 PLIX"
    )
    ...
    sender.accountBalances(dApp.address)._1 shouldBe dAppInitBalance - 0.009.plix - 0.053.plix
  ...
  test("dApp caller is dApp address") {
    ...
    val dAppPlixBalance = sender.accountBalances(dApp.stringRepr)._1
    ...
    sender.accountBalances(dApp.stringRepr)._1 shouldBe dAppPlixBalance - 0.009.plix
  }
