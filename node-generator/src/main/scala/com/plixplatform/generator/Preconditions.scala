...
import com.plixplatform.account.{Address, AddressScheme, KeyPair}
...
import com.plixplatform.transaction.assets.{IssueTransaction, IssueTransactionV2}
import com.plixplatform.transaction.lease.{LeaseTransaction, LeaseTransactionV2}
          ...
          action match {
            ...
            case CreateAccountP(seed, balance, scriptOption) =>
              ...
              val transferTx = TransferTransactionV2
                .selfSigned(Plix, settings.faucet, acc, balance, time.correctedTime(), Plix, Fee, "Generator".getBytes("UTF-8"))
                .explicitGet()
