...
object ExtensionPackaging extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[_]] =
    Seq(
      ...
      // To write files to Plix NODE directory
  ...
  private def maintainerFix = inConfig(Linux)(
    Seq(
      ...
      packageSummary := s"Plix node ${name.value}${network.value.packageSuffix} extension",
      packageDescription := s"Plix node ${name.value}${network.value.packageSuffix} extension"
    ))
