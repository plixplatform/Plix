package com.plixlatform.protobuf.transaction
import com.google.protobuf.ByteString
import com.plixlatform.account.{Address, AddressScheme, PublicKey}
import com.plixlatform.common.state.ByteStr
import com.plixlatform.lang.ValidationError
import com.plixlatform.lang.script.ScriptReader
import com.plixlatform.protobuf.transaction.Transaction.Data
import com.plixlatform.protobuf.transaction.{Script => PBScript}
import com.plixlatform.serialization.Deser
import com.plixlatform.state.{BinaryDataEntry, BooleanDataEntry, IntegerDataEntry, StringDataEntry}
...
import com.plixlatform.transaction.transfer.MassTransferTransaction
import com.plixlatform.transaction.transfer.MassTransferTransaction.ParsedTransfer
import com.plixlatform.transaction.{Proofs, TxValidationError}
import com.plixlatform.{transaction => vt}

object PBTransactions {
  import com.plixlatform.protobuf.utils.PBInternalImplicits._

  def create(sender: com.plixlatform.account.PublicKey = PublicKey.empty,
             chainId: Byte = 0,
             fee: Long = 0L,
             feeAssetId: VanillaAssetId = Plix,
             timestamp: Long = 0L,
             version: Int = 0,
             proofsArray: Seq[com.plixlatform.common.state.ByteStr] = Nil,
             data: com.plixlatform.protobuf.transaction.Transaction.Data = com.plixlatform.protobuf.transaction.Transaction.Data.Empty)
    : SignedTransaction = {
    ...
    val result: Either[ValidationError, VanillaTransaction] = data match {
      ...
      case Data.CreateAlias(CreateAliasTransactionData(alias)) =>
        version match {
          case 1 =>
            for {
              alias <- com.plixlatform.account.Alias.createWithChainId(alias, chainId)
      ...
      case Data.InvokeScript(InvokeScriptTransactionData(Some(dappAddress), functionCall, payments)) =>
        ...
        import com.plixlatform.lang.v1.compiler.Terms.FUNCTION_CALL
        ...
        version match {
          case 1 =>
            vt.CreateAliasTransactionV1(sender,
                                        com.plixlatform.account.Alias.createWithChainId(alias, chainId).explicitGet(),
                                        feeAmount,
                                        timestamp,
                                        signature)
  ...
  def protobuf(tx: VanillaTransaction): PBSignedTransaction = {
    ...
    tx match {
      ...
      case vt.PaymentTransaction(sender, recipient, amount, fee, timestamp, signature) =>
        ...
        PBTransactions.create(sender, chainId, fee, Plix, timestamp, 1, Seq(signature), Data.Payment(data))
  ...
  def toVanillaDataEntry(de: DataTransactionData.DataEntry): com.plixlatform.state.DataEntry[_] = {
  ...
  def toPBDataEntry(de: com.plixlatform.state.DataEntry[_]): DataTransactionData.DataEntry = {
