...
import com.plixplatform.account.{Address, AddressScheme, PublicKey}
import com.plixplatform.common.state.ByteStr
import com.plixplatform.lang.ValidationError
import com.plixplatform.lang.script.ScriptReader
import com.plixplatform.protobuf.transaction.Transaction.Data
import com.plixplatform.protobuf.transaction.{Script => PBScript}
...
import com.plixplatform.transaction.{Proofs, TxValidationError}
import com.plixplatform.{transaction => vt}

object PBTransactions {
  import com.plixplatform.protobuf.utils.PBInternalImplicits._

  def create(sender: com.plixplatform.account.PublicKey = PublicKey.empty,
             chainId: Byte = 0,
             fee: Long = 0L,
             feeAssetId: VanillaAssetId = Plix,
             timestamp: Long = 0L,
             version: Int = 0,
             proofsArray: Seq[com.plixplatform.common.state.ByteStr] = Nil,
             data: com.plixplatform.protobuf.transaction.Transaction.Data = com.plixplatform.protobuf.transaction.Transaction.Data.Empty)
    : SignedTransaction = {
    ...
    val result: Either[ValidationError, VanillaTransaction] = data match {
      ...
      case Data.CreateAlias(CreateAliasTransactionData(alias)) =>
        version match {
          case 1 =>
            for {
              alias <- com.plixplatform.account.Alias.createWithChainId(alias, chainId)
        ...
        version match {
          case 1 =>
            vt.CreateAliasTransactionV1(sender,
                                        com.plixplatform.account.Alias.createWithChainId(alias, chainId).explicitGet(),
                                        feeAmount,
                                        timestamp,
                                        signature)
  ...
  def toVanillaDataEntry(de: DataTransactionData.DataEntry): com.plixplatform.state.DataEntry[_] = {
  ...
  def toPBDataEntry(de: com.plixplatform.state.DataEntry[_]): DataTransactionData.DataEntry = {
