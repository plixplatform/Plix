...
import com.plixplatform.api.http.ApiError.InvalidAddress
...
import com.plixplatform.mining.{Miner, MinerDebugInfo}
import com.plixplatform.network.{LocalScoreChanged, PeerDatabase, PeerInfo, _}
import com.plixplatform.settings.{PlixSettings, RestAPISettings}
import com.plixplatform.state.diffs.TransactionDiffer
import com.plixplatform.state.extensions.Distributions
import com.plixplatform.state.{Blockchain, LeaseBalance, NG, TransactionId}
import com.plixplatform.transaction.TxValidationError.InvalidRequestSignature
import com.plixplatform.transaction._
import com.plixplatform.transaction.smart.script.trace.TracedResult
import com.plixplatform.transaction.smart.{InvokeScriptTransaction, Verifier}
...
@Path("/debug")
@Api(value = "/debug")
case class DebugApiRoute(ws: PlixSettings,
                         time: Time,
  ...
  private lazy val plixConfig: JsObject = Json.obj("plix" -> (fullConfig \ "plix").get)

  override val settings: RestAPISettings = ws.restAPISettings
  override lazy val route: Route = pathPrefix("debug") {
    blocks ~ state ~ info ~ statePlix ~ rollback ~ rollbackTo ~ blacklist ~ portfolios ~ minerInfo ~ historyInfo ~ configInfo ~ print ~ validate ~ stateChanges
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.plixplatform.http.DebugMessage",
        defaultValue = "{\n\t\"message\": \"foo\"\n}"
      )
    ))
  ...
  def state: Route = (path("state") & get & withAuth) {
    complete(dst.plixDistribution(ng.height).map(_.map { case (a, b) => a.stringRepr -> b }))
  }

  @Path("/statePlix/{height}")
  @ApiOperation(value = "State at block", notes = "Get state at specified height", httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "height", value = "height", required = true, dataType = "integer", paramType = "path")
    ))
  def statePlix: Route = (path("statePlix" / IntNumber) & get & withAuth) { height =>
    complete(dst.plixDistribution(height).map(_.map { case (a, b) => a.stringRepr -> b }))
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.plixplatform.http.RollbackParams",
  ...
  def configInfo: Route = (path("configInfo") & get & parameter('full.as[Boolean]) & withAuth) { full =>
    complete(if (full) fullConfig else plixConfig)
  }
