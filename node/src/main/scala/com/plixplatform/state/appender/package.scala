...
import com.plixplatform.transaction.TxValidationError.{BlockAppendError, BlockFromFuture, GenericError}
...
package object appender extends ScorexLogging {
  ...
  private val exceptions = List(
    ...
    262511 -> ByteStr.decodeBase58("4oJ1iC86ZBVxRTrFZuuoxentuTdTLQspSmVbXoWZgr6Fefev9yeyQF7BcxdBpX1MHR9WK4nLtin5aM5hWqjwPTBF").get
  )
  ...
  private def checkExceptions(height: Int, block: Block): Either[ValidationError, Unit] = {
    Either
      .cond(
        ...
        (),
        GenericError(s"Block time ${block.timestamp} on ${height} less than expected")
      )
  }
