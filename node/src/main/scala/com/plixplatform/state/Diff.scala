...
import com.plixplatform.account.{Address, Alias, PublicKey}
...
object Sponsorship {
  ...
  def calcPlixFeeAmount(tx: Transaction, getSponsorship: IssuedAsset => Option[Long]): Long = tx.assetFee match {
    case (asset @ IssuedAsset(_), amountInAsset) =>
      val sponsorship = getSponsorship(asset).getOrElse(0L)
      Sponsorship.toPlix(amountInAsset, sponsorship)

    case (Asset.Plix, amountInPlix) =>
      amountInPlix
  }
  ...
  def fromPlix(plixFee: Long, sponsorship: Long): Long = {
    if (plixFee == 0 || sponsorship == 0) return 0
    val assetFee = BigInt(plixFee) * sponsorship / FeeValidation.FeeUnit
