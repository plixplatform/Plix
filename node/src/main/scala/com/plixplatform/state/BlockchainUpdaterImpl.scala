...
import com.plixlatform.account.{Address, Alias}
import com.plixlatform.block.Block.BlockId
import com.plixlatform.block.{Block, BlockHeader, MicroBlock}
...
import com.plixlatform.metrics.{TxsInBlockchainStats, _}
import com.plixlatform.mining.{MiningConstraint, MiningConstraints, MultiDimensionalMiningConstraint}
import com.plixlatform.settings.{PlixSettings, BlockchainSettings}
import com.plixlatform.state.diffs.BlockDiffer
import com.plixlatform.state.extensions.composite.{CompositeAddressTransactions, CompositeDistributions}
import com.plixlatform.state.extensions.{AddressTransactions, Distributions}
import com.plixlatform.state.reader.{CompositeBlockchain, LeaseDetails}
import com.plixlatform.transaction.Asset.IssuedAsset
import com.plixlatform.transaction.TxValidationError.{BlockAppendError, GenericError, MicroBlockAppendError}
...
import kamon.Kamon
import kamon.metric.{CounterMetric, HistogramMetric, TimerMetric}
import monix.reactive.subjects.ReplaySubject
import monix.reactive.{Observable, Observer}

class BlockchainUpdaterImpl(private val blockchain: LevelDBWriter,
                            spendableBalanceChanged: Observer[(Address, Asset)],
                            PlixSettings: PlixSettings,
                            time: Time)
    extends BlockchainUpdater
    with NG
    with ScorexLogging {

  import com.plixlatform.state.BlockchainUpdaterImpl._
  import PlixSettings.blockchainSettings.functionalitySettings
  ...
  private lazy val maxBlockReadinessAge = PlixSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis
  ...
  override val settings: BlockchainSettings = PlixSettings.blockchainSettings
  ...
  private def featuresApprovedWithBlock(block: Block): Set[Short] = {
    ...
    if (height % featuresCheckPeriod == 0) {
      ...
      if (unimplementedActivated.nonEmpty) {
        ...
        if (PlixSettings.featuresSettings.autoShutdownOnUnsupportedFeature) {
  ...
  private def rewardForBlock(block: Block): Option[Long] = {
    ...
    blockchain
      ...
      .flatMap(currentReward => Some(currentReward))
  ...
  override def processBlock(block: Block, verify: Boolean = true): Either[ValidationError, Option[DiscardedTransactions]] = writeLock {
    ...
    Either
      .cond(
        !PlixSettings.featuresSettings.autoShutdownOnUnsupportedFeature || notImplementedFeatures.isEmpty,
        (),
                ...
                case Some((referencedForgedBlock, referencedLiquidDiff, carry, totalFee, discarded)) =>
                  if (!verify || referencedForgedBlock.signaturesValid().isRight) {
                    ...
                    val diff = BlockDiffer
                      .fromBlock(
                        CompositeBlockchain(blockchain, Some(referencedLiquidDiff), Some(referencedForgedBlock), carry, reward),
          ...
          _ map {
            case (BlockDiffer.Result(newBlockDiff, carry, totalFee, updatedTotalConstraint), discarded, reward) =>
              ...
              if ((block.timestamp > time
                    .getTimestamp() - PlixSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis) || (height % 100 == 0)) {
  ...
  override def plixAmount(height: Int): BigInt = readLock {
    ngState match {
      case Some(ng) if this.height == height =>
        blockchain.plixAmount(height - 1) + ng.reward.map(BigInt(_)).getOrElse(BigInt(0))
      case _ => blockchain.plixAmount(height)
    }
  }
  ...
  override def collectLposPortfolios[A](pf: PartialFunction[(Address, Portfolio), A]): Map[Address, A] = readLock {
    ngState.fold(blockchain.collectLposPortfolios(pf)) { ng =>
      ...
      for ((a, p) <- ng.bestLiquidDiff.portfolios if p.lease != LeaseBalance.empty || p.balance != 0) {
        pf.runWith(b += a -> _)(a -> this.plixPortfolio(a))
      }
  ...
  private[this] object metrics {
    val blockMicroForkStats: CounterMetric         = Kamon.counter("blockchain-updater.block-micro-fork")
    val microMicroForkStats: CounterMetric         = Kamon.counter("blockchain-updater.micro-micro-fork")
    val microBlockForkStats: CounterMetric         = Kamon.counter("blockchain-updater.micro-block-fork")
    val microBlockForkHeightStats: HistogramMetric = Kamon.histogram("blockchain-updater.micro-block-fork-height")
    val forgeBlockTimeStats: TimerMetric           = Kamon.timer("blockchain-updater.forge-block-time")
  }
}
