...
import com.plixplatform.block.Block.Fraction
...
object Portfolio {
  ...
  def combineAllPlix(pfs: (Address, Long)*): Map[Address, Portfolio] =
  ...
  def combineAllPlixOrAsset(asset: Asset)(pfs: (Address, Long)*): Map[Address, Portfolio] = asset match {
    case ia: IssuedAsset => combineAllAsset(ia)(pfs: _*)
    case Asset.Plix     => combineAllPlix(pfs: _*)
  }
