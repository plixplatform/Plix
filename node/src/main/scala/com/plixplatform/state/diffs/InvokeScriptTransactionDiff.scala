...
import com.plixlatform.lang.script.ContractScript.ContractScriptImpl
import com.plixlatform.lang.script.Script
import com.plixlatform.lang.v1.ContractLimits
...
import com.plixlatform.lang.v1.evaluator.{ContractEvaluator, LogItem, ScriptResult}
import com.plixlatform.lang.v1.traits.domain.Tx.ScriptTransfer
import com.plixlatform.lang.v1.traits.domain.{DataItem, Recipient}
import com.plixlatform.metrics._
import com.plixlatform.settings.Constants
import com.plixlatform.state._
import com.plixlatform.state.diffs.CommonValidation._
import com.plixlatform.state.diffs.FeeValidation._
...
import com.plixlatform.transaction.smart.script.ScriptRunner
import com.plixlatform.transaction.smart.script.ScriptRunner.TxOrd
import com.plixlatform.transaction.smart.script.trace.{AssetVerifierTrace, InvokeScriptTrace, TracedResult}
import com.plixlatform.transaction.smart.{InvokeScriptTransaction, PlixEnvironment}
      ...
      case Right(Some(sc @ ContractScriptImpl(_, contract, _))) =>
        val scriptResultE =
          stats.invokedScriptExecution.measureForType(InvokeScriptTransaction.typeId)({
            val environment = new PlixEnvironment(
                ...
                Monoid
                  .combineAll(
                    Seq(
                      ...
                      PlixContext.build(directives, environment)
                    )
                  )
                  .evaluationContext,
                contract,
        ...
        for {
          ...
          feeInfo <- TracedResult(tx.assetFee._1 match {
            case Plix => Right((tx.fee, Map(tx.sender.toAddress -> Portfolio(-tx.fee, LeaseBalance.empty, Map.empty))))
            case asset @ IssuedAsset(_) =>
              for {
                ...
                plixFee <- Either.cond(
                  ...
                  Sponsorship.toPlix(tx.fee, assetInfo.sponsorship),
                ...
                (plixFee,
                 Portfolio.combineAll(
                   ...
                   assetInfo.issuer.toAddress -> Portfolio(-plixFee, LeaseBalance.empty, Map(asset -> tx.fee))
                 ))
              }
          })
          ...
          _ <- TracedResult {
            ...
            val minPlix = totalScriptsInvoked * ScriptExtraFee + OldFeeUnits(InvokeScriptTransaction.typeId) * FeeUnit
            val txName   = Constants.TransactionNames(InvokeScriptTransaction.typeId)
            Either.cond(
              minPlix <= plixFee,
              (),
              GenericError(s"Fee in ${tx.assetFee._1
                .fold("PLIX")(_.toString)} for $txName with $totalScriptsInvoked total scripts invoked does not exceed minimal value of $minPlix PLIX: ${tx.assetFee._2}")
            )
          }
          scriptsInvoked <- TracedResult {
            ...
            Either.cond(
              ...
              totalScriptsInvoked,
              GenericError(s"Fee in ${tx.assetFee._1
                .fold("PLIX")(_.toString)} for ${tx.builder.classTag} with $totalScriptsInvoked total scripts invoked does not exceed minimal value of $minPlix PLIX: ${tx.assetFee._2}")
            )
          }
  ...
  private def payableAndDataPart(height: Int,
    ...
    if (dataEntries.length > ContractLimits.MaxWriteSetSize) {
      ...
      val payablePart = Monoid.combineAll(
        tx.payment.map(
          p =>
            Portfolio.combineAllPlixOrAsset(p.assetId)(
      ...
      val foldResult = ps.foldLeft(TracedResult(dataDiff.asRight[ValidationError])) { (tracedDiffAcc, payment) =>
        ...
        val tracedDiff: TracedResult[ValidationError, Diff] = Asset.fromCompatId(asset) match {
          case Plix =>
            Diff
              .stateOps(
                portfolios = Portfolio.combineAllPlix(
                  address     -> amount,
                  dAppAddress -> -amount
                )
              )
