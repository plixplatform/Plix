...
import com.plixlatform.transaction.smart._
import com.plixlatform.transaction.transfer._

object FeeValidation {

  case class FeeDetails(asset: Asset, requirements: Chain[String], minFeeInAsset: Long, minFeeInPlix: Long)
  ...
  val FeeUnits: Map[Byte, Long] = {
    val patches = Map[Byte, Long](
      DataTransaction.typeId -> 10 // 0.01 Plix
    )
    OldFeeUnits ++ patches
  }
  ...
  private def notEnoughFeeError(txType: Byte, feeDetails: FeeDetails, feeAmount: Long): ValidationError = {
    ...
    val actualFee   = s"$feeAmount in ${feeDetails.asset.fold("PLIX")(_.id.base58)}"
    val requiredFee = s"${feeDetails.minFeeInPlix} PLIX${feeDetails.asset.fold("")(id => s" or ${feeDetails.minFeeInAsset} ${id.id.base58}")}"
  ...
  private case class FeeInfo(assetInfo: Option[(IssuedAsset, AssetDescription)], requirements: Chain[String], plixFee: Long)
  ...
  private def feeAfterSponsorship(txAsset: Asset, height: Int, blockchain: Blockchain, tx: Transaction): Either[ValidationError, FeeInfo] = {
    if (height < Sponsorship.sponsoredFeesSwitchHeight(blockchain)) {
      ...
      for {
        ...
        r <- txAsset match {
          case Plix => Right(FeeInfo(None, Chain.empty, feeInUnits * FeeUnit))
          case assetId @ IssuedAsset(_) =>
            ...
            } yield FeeInfo(Some((assetId, assetInfo)), Chain.empty, plixFee)
        }
      } yield r
    }
  }
  ...
  def getMinFee(blockchain: Blockchain, height: Int, tx: Transaction): Either[ValidationError, FeeDetails] = {
    feeAfterSponsorship(tx.assetFee._1, height, blockchain, tx)
      ...
      .map {
        case FeeInfo(Some((assetId, assetInfo)), reqs, amountInPlix) =>
          FeeDetails(assetId, reqs, Sponsorship.fromPlix(amountInPlix, assetInfo.sponsorship), amountInPlix)
        case FeeInfo(None, reqs, amountInPlix) =>
          FeeDetails(Plix, reqs, amountInPlix, amountInPlix)
      }
  }
}
