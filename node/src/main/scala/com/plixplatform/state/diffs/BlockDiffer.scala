...
import com.plixlatform.state.patch.{CancelAllLeases, CancelInvalidLeaseIn, CancelLeaseOverflow}
import com.plixlatform.state.reader.CompositeBlockchain
import com.plixlatform.transaction.{Asset, Transaction}
import com.plixlatform.transaction.TxValidationError.{ActivationError, _}
...
object BlockDiffer extends ScorexLogging {
  ...
  def fromBlockTraced[Constraint <: MiningConstraint](blockchain: Blockchain,
    ...
    val minerRewardDistr: Portfolio =
      blockchain.lastBlockReward.map(Portfolio.build(Asset.Plix, _)).getOrElse(Portfolio.empty)
            ...
            balance = portfolio.balance +
              portfolio.assets.map {
                case (assetId, assetFee) =>
                  ...
                  Sponsorship.toPlix(assetFee, baseFee)
              }.sum)
        else portfolio
        ...
        case (TracedResult(Right(Result(currDiff, carryFee, currTotalFee, currConstraint)), _), tx) =>
          ...
          txDiffer(currBlockchain, tx).flatMap { newDiff =>
            ...
            if (updatedConstraint.isOverfilled)
              ...
              val totalPlixFee                = currTotalFee + curBlockFees.balance + nextBlockFee
              ...
              Right(Result(currDiff.combine(resultDiff), resultCarryFee, totalPlixFee, updatedConstraint))
            }
          }
      }
      .map { result =>
