...
object Exporter extends ScorexLogging {
  private[plixplatform] object Formats {
    val Binary   = "BINARY"
  ...
  private[this] final case class ExporterOptions(configFileName: File = new File("plix-testnet.conf"),
  ...
  private[this] lazy val commandParser = {
    ...
    OParser.sequence(
      programName("plix export"),
      head("Plix Blockchain Exporter", Version.VersionString),
      opt[File]('c', "config")
