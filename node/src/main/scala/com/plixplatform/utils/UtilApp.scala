...
import com.plixlatform.account.{KeyPair, PrivateKey, PublicKey}
import com.plixlatform.common.utils.{Base58, Base64, FastBase58}
...
import com.plixlatform.{Application, Version}
...
object UtilApp {
  ...
  private[this] lazy val commandParser = {
    ...
    OParser.sequence(
      programName("plix util"),
      head("Plix Util", Version.VersionString),
      OParser.sequence(
  ...
  private[this] object Actions {
    ...
    def doSign(c: Command, data: Array[Byte]): ActionResult =
      Right(com.plixlatform.crypto.sign(c.signOptions.privateKey, data))

    def doVerify(c: Command, data: Array[Byte]): ActionResult =
      Either.cond(com.plixlatform.crypto.verify(c.verifyOptions.signature, data, c.verifyOptions.publicKey), data, "Invalid signature")
    ...
    def doHash(c: Command, data: Array[Byte]): ActionResult = c.hashOptions.mode match {
      case "fast"   => Right(com.plixlatform.crypto.fastHash(data))
      case "secure" => Right(com.plixlatform.crypto.secureHash(data))
