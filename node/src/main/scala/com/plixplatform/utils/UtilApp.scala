...
import com.plixplatform.common.utils.{Base58, Base64, FastBase58}
...
import com.plixplatform.{Application, Version}
...
object UtilApp {
  ...
  private[this] object Actions {
    ...
    def doSign(c: Command, data: Array[Byte]): ActionResult =
      Right(com.plixplatform.crypto.sign(c.signOptions.privateKey, data))

    def doVerify(c: Command, data: Array[Byte]): ActionResult =
      Either.cond(com.plixplatform.crypto.verify(c.verifyOptions.signature, data, c.verifyOptions.publicKey), data, "Invalid signature")
    ...
    def doHash(c: Command, data: Array[Byte]): ActionResult = c.hashOptions.mode match {
      case "fast"   => Right(com.plixplatform.crypto.fastHash(data))
      case "secure" => Right(com.plixplatform.crypto.secureHash(data))
