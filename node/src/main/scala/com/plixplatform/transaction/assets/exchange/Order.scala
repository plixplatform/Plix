...
import com.plixplatform.serialization.{BytesSerializable, JsonSerializable}
...
import com.plixplatform.transaction.assets.exchange.Validation.booleanOperators
import com.plixplatform.utils.byteStrWrites
...
trait Order extends BytesSerializable with JsonSerializable with Proven {
  ...
  def matcherFeeAssetId: Asset = Plix
  ...
  def getSpendAmount(matchAmount: Long, matchPrice: Long): Either[ValidationError, Long] =
    Try {
      ...
      if (orderType == OrderType.SELL) matchAmount
        ...
        if (getSpendAssetId == Plix && !(spend + matcherFee).isValidLong) {
  ...
  override def toString: String = {
    val matcherFeeAssetIdStr = if (version == 3) s" matcherFeeAssetId=${matcherFeeAssetId.fold("Plix")(_.toString)}," else ""
