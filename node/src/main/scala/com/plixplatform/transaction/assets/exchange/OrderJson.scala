...
import com.plixlatform.transaction.{Asset, Proofs}
...
object OrderJson {
  ...
  private val assetReads: Reads[Asset] = {
    case JsNull | JsString("") => JsSuccess(Plix)
    case JsString(s) =>
  ...
  implicit val assetPairReads: Reads[AssetPair] = {
    val r = (JsPath \ "amountAsset").readWithDefault[Asset](Plix)(assetReads) and
      (JsPath \ "priceAsset").readWithDefault[Asset](Plix)(assetReads)
    r(AssetPair(_, _))
  }
