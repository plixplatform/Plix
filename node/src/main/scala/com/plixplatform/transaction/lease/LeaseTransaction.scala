...
import com.plixplatform.account.{Address, AddressOrAlias, PublicKey}
import com.plixplatform.lang.ValidationError
import com.plixplatform.transaction.Asset.Plix
import com.plixplatform.transaction.{Asset, ProvenTransaction, TxValidationError, VersionedTransaction}
...
object LeaseTransaction {
  ...
  def validateLeaseParams(amount: Long, fee: Long, recipient: AddressOrAlias, sender: PublicKey): Either[ValidationError, Unit] =
    if (amount <= 0) {
      Left(TxValidationError.NonPositiveAmount(amount, "plix"))
