...
import com.plixlatform.lang.v1.traits.domain.Recipient._
...
object PlixEnvironment {
  type In = Transaction :+: Order :+: ScriptTransfer :+: CNil
}

class PlixEnvironment(nByte: Byte, in: Coeval[PlixEnvironment.In], h: Coeval[Int], blockchain: Blockchain, address: Coeval[ByteStr])
    extends Environment {
  ...
  override def data(recipient: Recipient, key: String, dataType: DataType): Option[Any] = {
    for {
      address <- recipient match {
        case Address(bytes) =>
          com.plixlatform.account.Address
            .fromBytes(bytes.arr)
            .toOption
        case Alias(name) =>
          com.plixlatform.account.Alias
            .create(name)
  ...
  override def resolveAlias(name: String): Either[String, Recipient.Address] =
    blockchain
      .resolveAlias(com.plixlatform.account.Alias.create(name).explicitGet())
      .left
      .map(_.toString)
      .right
  ...
  override def accountBalanceOf(addressOrAlias: Recipient, maybeAssetId: Option[Array[Byte]]): Either[String, Long] = {
    (for {
      aoa <- addressOrAlias match {
        ...
        case Alias(name)    => com.plixlatform.account.Alias.create(name)
      }
