...
import com.plixlatform.block.BlockHeader
import com.plixlatform.common.state.ByteStr
import com.plixlatform.common.utils.EitherExt2
import com.plixlatform.lang.script.{Script, ScriptReader}
...
object Keys {
  ...
  def plixBalanceHistory(addressId: BigInt): Key[Seq[Int]] = historyKey("plix-balance-history", 5, addressId.toByteArray)
  def plixBalance(addressId: BigInt)(height: Int): Key[Long] =
    Key("plix-balance", hAddr(6, height, addressId), Option(_).fold(0L)(Longs.fromByteArray), Longs.toByteArray)
  ...
  val addressesForPlixSeqNr: Key[Int]                = intKey("addresses-for-plix-seq-nr", 37)
  def addressesForPlix(seqNr: Int): Key[Seq[BigInt]] = Key("addresses-for-plix", h(38, seqNr), readBigIntSeq, writeBigIntSeq)
  ...
  val plixAmountPrefix: Short              = 58
  def plixAmount(height: Int): Key[BigInt] = Key("plix-amount", h(plixAmountPrefix, height), Option(_).fold(BigInt(0))(BigInt(_)), _.toByteArray)
}
