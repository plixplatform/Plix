...
import com.plixlatform.metrics.LevelDBStats
import com.plixlatform.settings.DBSettings
...
import com.plixlatform.utils.{ObservedLoadingCache, ScorexLogging}
...
abstract class Caches(spendableBalanceChanged: Observer[(Address, Asset)]) extends Blockchain with ScorexLogging {
  ...
  def append(diff: Diff, carryFee: Long, totalFee: Long, reward: Option[Long], block: Block): Unit = {
    ...
    val plixBalances        = Map.newBuilder[BigInt, Long]
    ...
    for ((address, portfolioDiff) <- diff.portfolios) {
      ...
      if (portfolioDiff.balance != 0) {
        val wbalance = portfolioDiff.balance + balance(address, Plix)
        plixBalances += aid            -> wbalance
        newBalances += (address, Plix) -> wbalance
      }
