package com.plixlatform.database
...
import com.plixlatform.block.{Block, BlockHeader}
import com.plixlatform.common.state.ByteStr
import com.plixlatform.common.utils._
import com.plixlatform.database.patch.DisableHijackedAliases
...
import com.plixlatform.settings.{BlockchainSettings, Constants, DBSettings, FunctionalitySettings, GenesisSettings, RewardsSettings}
import com.plixlatform.state.extensions.{AddressTransactions, Distributions}
import com.plixlatform.state.reader.LeaseDetails
import com.plixlatform.state.{TxNum, _}
import com.plixlatform.transaction.Asset.{Plix, IssuedAsset}
import com.plixlatform.transaction.TxValidationError.{AliasDoesNotExist, AliasIsDisabled, GenericError}
import com.plixlatform.transaction._
import com.plixlatform.transaction.assets._
import com.plixlatform.transaction.assets.exchange.ExchangeTransaction
import com.plixlatform.transaction.lease.{LeaseCancelTransaction, LeaseTransaction}
...
class LevelDBWriter(private[database] val writableDB: DB,
  ...
  protected override def loadBalance(req: (Address, Asset)): Long = readOnly { db =>
    addressId(req._1).fold(0L) { addressId =>
      req._2 match {
        ...
        case Plix                  => db.fromHistory(Keys.plixBalanceHistory(addressId), Keys.plixBalance(addressId)).getOrElse(0L)
      }
    }
  }
  ...
  private[database] def loadLposPortfolio(db: ReadOnlyDB, addressId: BigInt) = Portfolio(
    db.fromHistory(Keys.plixBalanceHistory(addressId), Keys.plixBalance(addressId)).getOrElse(0L),
  ...
  override def plixAmount(height: Int): BigInt = readOnly { db =>
    if (db.has(Keys.plixAmount(height))) db.get(Keys.plixAmount(height))
    else BigInt(Constants.UnitsInWave * Constants.TotalPlix)
  }
  ...
  override protected def doAppend(block: Block,
    ...
    val newAddressesForPlix = ArrayBuffer.empty[BigInt]
    val updatedBalanceAddresses = for ((addressId, balance) <- plixBalances) yield {
      val kwbh = Keys.plixBalanceHistory(addressId)
      val wbh  = rw.get(kwbh)
      if (wbh.isEmpty) {
        newAddressesForPlix += addressId
      }
      rw.put(Keys.plixBalance(addressId)(height), balance)
      expiredKeys ++= updateHistory(rw, wbh, kwbh, balanceThreshold, Keys.plixBalance(addressId))
      addressId
    }

    val changedAddresses = addressTransactions.keys ++ updatedBalanceAddresses

    if (newAddressesForPlix.nonEmpty) {
      val newSeqNr = rw.get(Keys.addressesForPlixSeqNr) + 1
      rw.put(Keys.addressesForPlixSeqNr, newSeqNr)
      rw.put(Keys.addressesForPlix(newSeqNr), newAddressesForPlix)
    }
    ...
    reward.foreach { lastReward =>
      ...
      rw.put(Keys.plixAmount(height), plixAmount(height - 1) + lastReward)
    }
      ...
      val discardedBlocks: Seq[Block] = for (currentHeight <- height until targetHeight by -1) yield {
        ...
        val discardedBlock = readWrite { rw =>
          ...
          for (aId <- rw.get(Keys.changedAddresses(currentHeight))) {
            ...
            balancesToInvalidate += (address -> Plix)
            ...
            rw.delete(Keys.plixBalance(addressId)(currentHeight))
            rw.filterHistory(Keys.plixBalanceHistory(addressId), currentHeight)
          ...
          rw.delete(Keys.plixAmount(currentHeight))
  ...
  override def balanceSnapshots(address: Address, from: Int, to: BlockId): Seq[BalanceSnapshot] = readOnly { db =>
    db.get(Keys.addressId(address)).fold(Seq(BalanceSnapshot(1, 0, 0, 0))) { addressId =>
      ...
      val wbh      = slice(db.get(Keys.plixBalanceHistory(addressId)), from, toHeigth)
      val lbh      = slice(db.get(Keys.leaseBalanceHistory(addressId)), from, toHeigth)
      for {
        (wh, lh) <- merge(wbh, lbh)
        wb = balanceAtHeightCache.get((wh, addressId), () => db.get(Keys.plixBalance(addressId)(wh)))
