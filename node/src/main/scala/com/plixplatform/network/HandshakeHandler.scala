...
import com.plixplatform.network.Handshake.InvalidHandshakeException
...
object HandshakeHandler extends ScorexLogging {
  ...
  def checkApplicationName(local: String, remote: String): Boolean = {
    val appName = remote.substring(0,4)
    val chainId = remote.substring(4)
    val localChainId = local.substring(4)

    (appName == "plix") && (chainId == "A" || chainId == "K" || chainId == "D" || chainId == "I") && chainId == localChainId
  }
