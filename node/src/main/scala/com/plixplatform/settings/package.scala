...
package object settings {
  ...
  def loadConfig(maybeUserConfig: Option[Config]): Config = {
    ...
    val cmdDefaults =
      Try(withApp.getConfig("plix.defaults"))
        .getOrElse(ConfigFactory.empty())
        .atPath("plix")

    external
      .withFallback(cmdDefaults)
      .withFallback(ConfigFactory.parseString(s"plix.directory = ${defaultDirectory(withApp)}"))
      .withFallback(ConfigFactory.defaultApplication())
      .withFallback(ConfigFactory.defaultReference())
      .resolve()
  }
  ...
  def defaultDirectory(config: Config): String = {
    ...
    def withNetwork(config: Config): String = {
      val bc = config.getString("plix.blockchain.type")
      val suffix =
        if (bc == "CUSTOM") {
          val char = config.getString("plix.blockchain.custom.address-scheme-character").headOption.getOrElse(0.toChar)
