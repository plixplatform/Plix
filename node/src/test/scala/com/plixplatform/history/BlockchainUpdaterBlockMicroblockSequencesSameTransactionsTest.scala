...
class BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest
      ...
      case (gen, rest) =>
        val finalMinerBalances = rest.map {
          case (a @ (bmb: BlockAndMicroblockSequence, last: Block)) =>
            withDomain(MicroblocksActivatedAt0PlixSettings) { d =>
  ...
  property("Miner fee from microblock [Genesis] <- [Empty] <~ (Micro with tx) <- [Empty]") {
    val preconditionsAndPayments: Gen[(KeyPair, GenesisTransaction, TransferTransactionV1, Int)] = for {
      ...
      payment: TransferTransactionV1 = createPlixTransfer(master, master, amt, fee, ts).explicitGet()
  ...
  def randomPayment(accs: Seq[KeyPair], ts: Long): Gen[TransferTransactionV1] =
    for {
      ...
      fee  <- smallFeeGen
      amt  <- smallFeeGen
    } yield createPlixTransfer(from, to, amt, fee, ts).explicitGet()
  ...
  def accsAndGenesis(): Gen[(Seq[KeyPair], KeyPair, Block, Int)] =
    for {
      ...
      miner   <- accountGen
      ts      <- positiveIntGen
      genesis1: GenesisTransaction = GenesisTransaction.create(alice, TOTAL_PLIX / 4, ts).explicitGet()
      genesis2: GenesisTransaction = GenesisTransaction.create(bob, TOTAL_PLIX / 4, ts + 1).explicitGet()
      genesis3: GenesisTransaction = GenesisTransaction.create(charlie, TOTAL_PLIX / 4, ts + 2).explicitGet()
      genesis4: GenesisTransaction = GenesisTransaction.create(dave, TOTAL_PLIX / 4, ts + 4).explicitGet()
    } yield
