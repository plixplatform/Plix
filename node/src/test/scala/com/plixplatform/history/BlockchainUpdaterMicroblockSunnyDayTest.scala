...
import com.plixplatform.account.{KeyPair, Address, AddressOrAlias}
...
class BlockchainUpdaterMicroblockSunnyDayTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransactionV1 <- plixTransferGeneratorP(master, alice)
    aliceToBob  = createPlixTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts).explicitGet()
    aliceToBob2 = createPlixTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts + 1).explicitGet()
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) {
      case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
        ...
        val minerABalance = withDomain(MicroblocksActivatedAt0PlixSettings) { da =>
        ...
        val minerBBalance = withDomain(MicroblocksActivatedAt0PlixSettings) { db =>
