...
import com.plixplatform.settings.{Constants, FunctionalitySettings, RewardsSettings}
...
class BlockRewardSpec extends FreeSpec with ScalaCheckPropertyChecks with WithDomain with Matchers with TransactionGen with NoShrink {
  ...
  private val genesis = for {
    ...
    genesisBlock = TestBlock.create(
      ...
      Seq(
        GenesisTransaction.create(sourceAddress, (Constants.TotalPlix - 60000) * Constants.UnitsInWave, ntpTime.getTimestamp()).explicitGet(),
  ...
  "Miner receives reward and fees" - {
    val ngEmptyScenario = for {
      ...
      tx1 = TransferTransactionV1
        .selfSigned(Plix, issuer, sourceAddress, 10 * Constants.UnitsInWave, ntpTime.getTimestamp(), Plix, OneTotalFee, Array.emptyByteArray)
        .explicitGet()
