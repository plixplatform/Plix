...
import com.plixlatform.db.DBCacheSettings
...
import com.plixlatform.settings.{TestFunctionalitySettings, PlixSettings, loadConfig}
import com.plixlatform.state.diffs.ENOUGH_AMT
import com.plixlatform.state.utils.BlockchainAddressTransactionsList
import com.plixlatform.state.{BlockchainUpdaterImpl, Height}
import com.plixlatform.transaction.Asset.Plix
import com.plixlatform.transaction.lease.{LeaseCancelTransactionV1, LeaseTransaction}
import com.plixlatform.transaction.smart.SetScriptTransaction
import com.plixlatform.transaction.transfer.{TransferTransaction, TransferTransactionV1}
import com.plixlatform.transaction.{GenesisTransaction, Transaction}
import com.plixlatform.utils.Time
import com.plixlatform.{RequestGen, TransactionGen, WithDB}
...
class LevelDBWriterSpec extends FreeSpec with Matchers with TransactionGen with WithDB with DBCacheSettings with RequestGen {
  ...
  def createTransfer(master: KeyPair, recipient: Address, ts: Long): TransferTransaction = {
    TransferTransactionV1
      .selfSigned(Plix, master, recipient, ENOUGH_AMT / 5, ts, Plix, 1000000, Array.emptyByteArray)
      .explicitGet()
  }
