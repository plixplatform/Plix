...
import com.plixlatform.lang.{Global, Testing}
  ...
  val preconditions: Gen[
    ...
    for {
      ...
      transferFromCompanyToA = TransferTransactionV1
        .selfSigned(assetId, company, accountA, 1, ts + 20, Plix, 1000, Array.empty)
        .explicitGet()

      transferFromAToB = TransferTransactionV1
        .selfSigned(assetId, accountA, accountB, 1, ts + 30, Plix, 1000, Array.empty)
        .explicitGet()
