...
import com.plixlatform.lang.directives.values.{Expression, V1}
...
class AssetTransactionsDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithDB {
  ...
  property("Issue+Reissue+Burn do not break plix invariant and updates state") {
  ...
  property("Can burn non-owned alias if feature 'BurnAnyTokens' activated") {
    val setup = for {
      ...
      assetTransfer <- transferGeneratorP(issuer, burner, IssuedAsset(issue.assetId), Plix)
      plixTransfer <- plixTransferGeneratorP(issuer, burner)
      burn = BurnTransactionV1.selfSigned(burner, IssuedAsset(issue.assetId), assetTransfer.amount, plixTransfer.amount, timestamp).explicitGet()
    } yield (genesis, issue, assetTransfer, plixTransfer, burn)
    ...
    forAll(setup) {
      case (genesis, issue, assetTransfer, plixTransfer, burn) =>
        assertDiffAndState(Seq(TestBlock.create(Seq(genesis, issue, assetTransfer, plixTransfer))), TestBlock.create(Seq(burn)), fs) {
          case (_, newState) =>
  ...
  property("Can not reissue > long.max") {
    val setup = for {
      ...
      reissue = ReissueTransactionV1.selfSigned(issuer, assetId, Long.MaxValue, reissuable = true, 1, timestamp).explicitGet()
  ...
  property("Can not total issue > long.max") {
    val setup = for {
      ...
      transfer = TransferTransactionV1.selfSigned(assetId, issuer, holder, quantity - 1, timestamp, Plix, fee, attachment).explicitGet()
      reissue  = ReissueTransactionV1.selfSigned(issuer, assetId, (Long.MaxValue - quantity) + 1, reissuable = true, 1, timestamp).explicitGet()
  ...
  def genesisIssueTransferReissue(code: String): Gen[(Seq[GenesisTransaction], IssueTransactionV2, TransferTransactionV1, ReissueTransactionV1)] =
    for {
      ...
      timestamp          <- timestampGen
      initialPlixAmount <- Gen.choose(Long.MaxValue / 1000, Long.MaxValue / 100)
      accountA           <- accountGen
      accountB           <- accountGen
      smallFee           <- Gen.choose(1l, 10l)
      genesisTx1 = GenesisTransaction.create(accountA, initialPlixAmount, timestamp).explicitGet()
      genesisTx2 = GenesisTransaction.create(accountB, initialPlixAmount, timestamp).explicitGet()
      reissuable = true
      ...
      transfer = TransferTransactionV1
        .selfSigned(assetId, accountA, accountB, issue.quantity, timestamp + 2, Plix, smallFee, Array.empty)
        .explicitGet()
