...
import com.plixlatform.account.{Address, AddressScheme, Alias, KeyPair}
...
import com.plixlatform.lang.contract.DApp.{CallableAnnotation, CallableFunction}
import com.plixlatform.lang.directives.DirectiveSet
import com.plixlatform.lang.directives.values.{DApp => DAppType, _}
import com.plixlatform.lang.script.{ContractScript, Script}
import com.plixlatform.lang.script.v1.ExprScript
import com.plixlatform.lang.v1.FunctionHeader.{Native, User}
import com.plixlatform.lang.v1.compiler.Terms
import com.plixlatform.lang.v1.compiler.Terms._
import com.plixlatform.lang.v1.evaluator.FunctionIds.{CREATE_LIST, THROW}
import com.plixlatform.lang.v1.evaluator.ctx.impl.plix.{PlixContext, FieldNames}
import com.plixlatform.lang.v1.evaluator.ctx.impl.{CryptoContext, PureContext}
import com.plixlatform.lang.v1.evaluator.{FunctionIds, ScriptResult}
import com.plixlatform.lang.v1.parser.{Expressions, Parser}
import com.plixlatform.lang.v1.{ContractLimits, FunctionHeader, compiler}
import com.plixlatform.lang.{Global, utils}
import com.plixlatform.settings.TestFunctionalitySettings
import com.plixlatform.state._
import com.plixlatform.state.utils._
import com.plixlatform.transaction.Asset.{Plix, IssuedAsset}
import com.plixlatform.transaction.TxValidationError.TransactionNotAllowedByScript
import com.plixlatform.transaction.assets._
import com.plixlatform.transaction.smart.InvokeScriptTransaction.Payment
import com.plixlatform.transaction.smart.script.trace.{AssetVerifierTrace, InvokeScriptTrace}
import com.plixlatform.transaction.smart.{PlixEnvironment, InvokeScriptTransaction, SetScriptTransaction}
import com.plixlatform.transaction.transfer.TransferTransactionV2
import com.plixlatform.transaction.{Asset, _}
...
class InvokeScriptTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithDB with Inside {
  ...
  def paymentContract(senderBinding: String,
                      ...
                      assets: List[Asset] = List(Plix)): DApp = {
  ...
  def paymentContractGen(address: Address, amount: Long, assets: List[Asset] = List(Plix))(func: String): Gen[DApp] =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
    } yield paymentContract(senderBinging, argBinding, func, address, amount, assets)

  def defaultPaymentContractGen(address: Address, amount: Long, assets: List[Asset] = List(Plix))(someName: String): Gen[DApp] =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
  ...
  def preconditionsAndSetContract(senderBindingToContract: String => Gen[DApp],
    ...
    for {
      ...
      sponsoredFee = Sponsorship.fromPlix(900000L, sponsorTx.minSponsoredAssetFee.get)
      ...
      ci = InvokeScriptTransaction
        .selfSigned(
          ...
          if (sponsored) sponsorTx.asset else Plix,
          ts + 3
        )
        .explicitGet()
    ...
    forAll(for {
      ...
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffAndState(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
          case (blockDiff, newState) =>
            blockDiff.scriptsRun shouldBe 1
            newState.balance(acc, Plix) shouldBe amount
        }
    }
  }
  ...
  property("can't make more than 10 payments") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = (paymentContractGen(a, am, List.fill(11)(Plix)) _)
      r <- preconditionsAndSetContract(contractGen)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  val enoughFee: Long = FeeValidation.ScriptExtraFee + FeeValidation.OldFeeUnits(IssueTransactionV2.typeId) * FeeValidation.FeeUnit
      ...
      case (acc, amount, genesis, setScript, ci, asset, master, ts) =>
        val t =
          TransferTransactionV2
            .selfSigned(IssuedAsset(asset.id()), master, acc, asset.quantity / 10, ts, Plix, enoughFee, Array[Byte]())
            .explicitGet()
  ...
  property("payment should be positive") {
    forAll(for {
      ...
      ci = InvokeScriptTransaction.selfSigned(invoker, master, Some(fc), Seq(Payment(-1, Plix)), fee, Plix, ts)
  ...
  property("can't overflow payment + fee") {
    forAll(for {
      ...
      r <- preconditionsAndSetContract(contractGen,
                                       invokerGen = Gen.oneOf(Seq(invoker)),
                                       payment = Some(Payment(Long.MaxValue, Plix)),
                                       feeGen = ciFee(1))
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("can't overflow sum of payment in contract") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = (paymentContractGen(a, Long.MaxValue / 2 + 2, List.fill(4)(Plix)) _)
      invoker <- accountGen
      ts      <- timestampGen
      r       <- preconditionsAndSetContract(contractGen, invokerGen = Gen.oneOf(Seq(invoker)), payment = Some(Payment(1, Plix)), feeGen = ciFee(1))
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
          _ should produce("Attempt to transfer unavailable funds")
  ...
  property("invoking contract with sponsored fee") {
    forAll(for {
      ...
      case (ts, acc, amount, genesis, setScript, ci, master, sponsoredAsset, setSponsorship) =>
        val t =
          TransferTransactionV2
            .selfSigned(IssuedAsset(sponsoredAsset.id()), master, ci.sender, sponsoredAsset.quantity / 10, ts, Plix, enoughFee, Array[Byte]())
            .explicitGet()
  ...
  property("Default function invocation should produce error if contract default function has arguments") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "default", a, am, List(Plix))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("Default function invocation should produce error if contract does't have default function") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "undefault", a, am, List(Plix))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
