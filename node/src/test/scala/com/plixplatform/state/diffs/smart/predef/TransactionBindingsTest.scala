package com.plixlatform.state.diffs.smart.predef
...
import com.plixlatform.common.state.diffs.ProduceError._
import com.plixlatform.lang.Testing.evaluated
import com.plixlatform.lang.directives.values._
import com.plixlatform.lang.directives.DirectiveSet
import com.plixlatform.lang.v1.compiler
import com.plixlatform.lang.v1.compiler.ExpressionCompiler
import com.plixlatform.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_BYTESTR, CONST_LONG, CONST_STRING, EVALUATED}
import com.plixlatform.lang.v1.evaluator.EvaluatorV1
...
import com.plixlatform.lang.Global
import com.plixlatform.state._
import com.plixlatform.transaction.Asset.Plix
import com.plixlatform.transaction.assets.exchange.{Order, OrderType}
import com.plixlatform.transaction.smart.BlockchainContext.In
import com.plixlatform.transaction.smart.PlixEnvironment
import com.plixlatform.transaction.{Proofs, ProvenTransaction, VersionedTransaction}
import com.plixlatform.utils.EmptyBlockchain
import com.plixlatform.{NoShrink, TransactionGen, crypto}
...
class TransactionBindingsTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
    ...
    forAll(invokeScriptGen) { t =>
      ...
      val script =
        s"""
            ...
            |     then if (${t.payment.headOption.exists(_.assetId != Plix)})
            ...
            | }
            |""".stripMargin
      val result = runScriptWithCustomContext(script, Coproduct(t), K, V3)
      result shouldBe evaluated(true)
    }
  }
  ...
  property("MassTransferTransaction binding") {
    forAll(massTransferGen) { t =>
      ...
      val script = s"""
                      ...
                      |    let assetId = if (${t.assetId != Plix}) then extract(t.assetId) == base58'${t.assetId.maybeBase58Repr
                        .getOrElse("")}'
                      |      else isDefined(t.assetId) == false
    ...
    forAll(exchangeTransactionGen) { t =>
      def pg(ord: Order) = {
        ...
        val script = s"""
           ...
           |   let ${oType}AssetPairAmount = if (${ord.assetPair.amountAsset != Plix}) then extract(t.${oType}Order.assetPair.amountAsset) == base58'${ord.assetPair.amountAsset.maybeBase58Repr
                          .getOrElse("")}'
           |   else isDefined(t.${oType}Order.assetPair.amountAsset) == false
           |   let ${oType}AssetPairPrice = if (${ord.assetPair.priceAsset != Plix}) then extract(t.${oType}Order.assetPair.priceAsset) == base58'${ord.assetPair.priceAsset.maybeBase58Repr
                          .getOrElse("")}'
           |   else isDefined(t.${oType}Order.assetPair.priceAsset) == false
           |   let ${oType}MatcherFeeAssetId = if (${ord.matcherFeeAssetId != Plix}) then extract(t.${oType}Order.matcherFeeAssetId) == base58'${ord.matcherFeeAssetId.maybeBase58Repr
                          .getOrElse("")}'
  ...
  property("Order binding") {
    forAll(orderGen) { t =>
      val s = s"""
                 ...
                 |   ${Range(0, 8).map(letProof(t.proofs, "t")).mkString("\n")}
                 |   let assetPairAmount = if (${t.assetPair.amountAsset != Plix}) then extract(t.assetPair.amountAsset) == base58'${t.assetPair.amountAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.amountAsset) == false
                 |   let assetPairPrice = if (${t.assetPair.priceAsset != Plix}) then extract(t.assetPair.priceAsset) == base58'${t.assetPair.priceAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.priceAsset) == false
                 |   let matcherFeeAssetId = if (${t.matcherFeeAssetId != Plix}) then extract(t.matcherFeeAssetId) == base58'${t.matcherFeeAssetId.maybeBase58Repr
                   .getOrElse("")}'
  ...
  property("Order type bindings") {
    forAll(orderGen) { ord =>
      ...
      runScript(src, Coproduct[In](ord), K) shouldBe an[Left[_, _]]
  ...
  def runForAsset(script: String): Either[String, EVALUATED] = {
    ...
    import com.plixlatform.lang.v1.CTX._
      ...
      PureContext
        ...
        PlixContext
          .build(
            ...
            new PlixEnvironment(chainId, Coeval(null), null, EmptyBlockchain, Coeval(null))
          )
      ...
      PureContext.build(Global, V2) |+|
        ...
        PlixContext
          .build(
            ...
            new PlixEnvironment(chainId, Coeval(t), null, EmptyBlockchain, Coeval(null))
          )
