...
import com.plixplatform.transaction.assets.{IssueTransactionV1, IssueTransactionV2, SponsorFeeTransaction}
import com.plixplatform.transaction.smart.SetScriptTransaction
import com.plixplatform.transaction.transfer._
import com.plixplatform.transaction.{GenesisTransaction, Transaction}
...
class CommonValidationTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with WithState with NoShrink {
  ...
  property("disallows double spending") {
    val preconditionsAndPayment: Gen[(GenesisTransaction, TransferTransactionV1)] = for {
      ...
      transfer: TransferTransactionV1 <- plixTransferGeneratorP(master, recipient)
  ...
  private def sponsorAndSetScriptGen(sponsorship: Boolean, smartToken: Boolean, smartAccount: Boolean, feeInAssets: Boolean, feeAmount: Long) =
    for {
      ...
      val transferPlixTx = TransferTransactionV1
        .selfSigned(Plix, richAcc, recipientAcc, 10 * Constants.UnitsInWave, ts, Plix, 1 * Constants.UnitsInWave, Array.emptyByteArray)
        .explicitGet()
      ...
      val transferBackTx = TransferTransactionV1
        .selfSigned(
          ...
          1,
          ts,
          if (feeInAssets) IssuedAsset(issueTx.id()) else Plix,
          feeAmount,
          Array.emptyByteArray
        )
        .explicitGet()

      (TestBlock.create(Vector[Transaction](genesisTx, issueTx, transferPlixTx, transferAssetTx) ++ sponsorTx ++ setScriptTx), transferBackTx)
    }
