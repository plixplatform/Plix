...
import com.plixplatform.settings.{Constants, FunctionalitySettings, TestFunctionalitySettings}
...
import com.plixplatform.transaction.assets.{IssueTransactionV1, SponsorFeeTransaction}
...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("not enough fee") {
    ...
    val setup = for {
      ...
      assetOverspend = TransferTransactionV1
        .selfSigned(Plix, master, recipient.toAddress, 1000000, ts + 1, IssuedAsset(assetId), issueTx.quantity + 1, Array.emptyByteArray)
        .right
        .get
      ...
      plixOverspend = TransferTransactionV1
        .selfSigned(Plix, master, recipient.toAddress, 1000000, ts + 3, IssuedAsset(assetId), fee, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issueTx, sponsorTx, assetOverspend, insufficientFee, plixOverspend)

    forAll(setup) {
      case (genesis, issue, sponsor, assetOverspend, insufficientFee, plixOverspend) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          val minFee = Sponsorship
            .fromPlix(
              FeeValidation.OldFeeUnits(insufficientFee.builder.typeId) * FeeValidation.FeeUnit,
              sponsor.minSponsoredAssetFee.get
            )

          val expectedError =
            s"Fee for TransferTransaction (${insufficientFee.fee} in ${issue.assetId.base58})" ++
              s" does not exceed minimal value of 100000 PLIX or $minFee ${issue.assetId.base58}"

          blockDiffEi should produce(expectedError)
        }
        assertDiffEi(setupBlocks, block(Seq(plixOverspend)), s) { blockDiffEi =>
          if (plixOverspend.fee > issue.quantity)
            blockDiffEi should produce("unavailable funds")
          else
            blockDiffEi should produce("negative plix balance")
        }
    }
  }

  property("not enough plix to pay fee after leasing") {
    val s = settings(0)
    val setup = for {
      master <- accountGen
      alice  <- accountGen
      ...
      transferAssetTx: TransferTransactionV1 = TransferTransactionV1
        .selfSigned(IssuedAsset(assetId), master, alice.toAddress, issueTx.quantity, ts + 2, Plix, fee, Array.emptyByteArray)
        .right
        .get
  ...
  property("cannot cancel sponsorship") {
    ...
    forAll(setup) {
      case (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("(99999999 in PLIX) does not exceed minimal value of 100000000 PLIX")
        }
    }
  }
  ...
  property("sponsor has no PLIX but receives them just in time") {
    val s = settings(0)
    val setup = for {
      master    <- accountGen
      recipient <- accountGen
      ...
      assetTransfer = TransferTransactionV1
        .selfSigned(assetId, master, recipient, issue.quantity, ts + 3, Plix, 100000, Array.emptyByteArray)
        .right
        .get
      plixTransfer = TransferTransactionV1
        .selfSigned(Plix, master, recipient, 99800000, ts + 4, Plix, 100000, Array.emptyByteArray)
        .right
        .get
      backPlixTransfer = TransferTransactionV1
        .selfSigned(Plix, recipient, master, 100000, ts + 5, assetId, 100, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issue, sponsor, assetTransfer, plixTransfer, backPlixTransfer)

    forAll(setup) {
      case (genesis, issue, sponsor, assetTransfer, plixTransfer, backPlixTransfer) =>
        assertDiffAndState(Seq(block(Seq(genesis, issue, sponsor, assetTransfer, plixTransfer))), block(Seq(backPlixTransfer)), s) {
          case (_, state) =>
