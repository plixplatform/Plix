...
import com.plixlatform.common.state._
...
object ScriptsCountTest {
  def calculateLegacy(blockchain: Blockchain, tx: Transaction): Int = {
    ...
    import com.plixlatform.transaction.assets.{BurnTransaction, ReissueTransaction, SponsorFeeTransaction}
    ...
    import com.plixlatform.transaction.{Authorized, Transaction}
  ...
  property("check pre-Ride4DApps scripts run count") {
    forAll(for {
      ...
      tr1 = TransferTransactionV2
        .selfSigned(Plix, master, acc, 10000000000L, timestamp, Plix, fee, ByteStr(Array()))
        .explicitGet()
      tr2 = TransferTransactionV2
        .selfSigned(IssuedAsset(issueScr.id()), master, acc, 1000000000L, timestamp, Plix, fee, ByteStr(Array()))
        .explicitGet()
      mt1 = MassTransferTransaction.selfSigned(Plix, master, List(ParsedTransfer(acc, 1)), timestamp, fee, ByteStr(Array())).explicitGet()
