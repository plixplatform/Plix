...
import com.plixlatform.account.{AddressScheme, KeyPair, PrivateKey, PublicKey}
import com.plixlatform.common.state.ByteStr
import com.plixlatform.common.utils.{Base58, EitherExt2}
import com.plixlatform.features.{BlockchainFeature, BlockchainFeatures}
...
import com.plixlatform.state.diffs.ExchangeTransactionDiff.getOrderFeePortfolio
...
import com.plixlatform.transaction.TxValidationError.AccountBalanceError
import com.plixlatform.transaction._
import com.plixlatform.transaction.assets.exchange.OrderOps._
import com.plixlatform.transaction.assets.exchange.{Order, _}
import com.plixlatform.transaction.assets.{IssueTransaction, IssueTransactionV1, IssueTransactionV2}
...
import com.plixlatform.transaction.transfer.{MassTransferTransaction, TransferTransaction}
...
class ExchangeTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with Inside with NoShrink {
  ...
  property("Preserves plix invariant, stores match info, rewards matcher") {
  ...
  property("Preserves assets invariant (matcher's fee in one of the assets of the pair or in Plix), stores match info, rewards matcher") {
            ...
            val restoredMatcherPortfolio =
              Monoid.combineAll(
                Seq(
                  ...
                  ExchangeTransactionDiff.plixPortfolio(-exchange.fee)
                )
              )
  ...
  property("buy plix without enough money for fee") {
    val preconditions: Gen[(GenesisTransaction, GenesisTransaction, IssueTransactionV1, ExchangeTransaction)] = for {
      ...
      exchange <- Gen.oneOf(
        exchangeV1GeneratorP(buyer, seller, Plix, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000)),
        exchangeV2GeneratorP(buyer, seller, Plix, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000))
      )
    } yield {
  ...
  property("small fee cases") {
    ...
    forAll(preconditions, priceGen) {
      case ((buyer, seller, matcher, gen1, gen2, issue1), price) =>
        val assetPair = AssetPair(IssuedAsset(issue1.id()), Plix)
  ...
  property("Diff for ExchangeTransaction works as expected and doesn't use rounding inside") {
    ...
    val assetPair                                          = AssetPair(Plix, IssuedAsset(issue1.id()))
  ...
  property("Disable use OrderV1 on SmartAccount") {
    ...
    forAll(for {
      ...
      tr1     = createPlixTransfer(MATCHER, buyer.toAddress, Long.MaxValue / 3, enoughFee, ts + 1).explicitGet()
      tr2     = createPlixTransfer(MATCHER, seller.toAddress, Long.MaxValue / 3, enoughFee, ts + 2).explicitGet()
