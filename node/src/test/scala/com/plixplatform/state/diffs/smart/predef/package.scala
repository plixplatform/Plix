package com.plixlatform.state.diffs.smart
...
import com.plixlatform.transaction.smart.BlockchainContext
...
package object predef {
  ...
  def scriptWithAllV1Functions(tx: DataTransaction, t: TransferTransaction): String =
    s"""${dropLastLine(scriptWithV1PureFunctions(tx, t))}
       |${dropLastLine(scriptWithV1PlixFunctions(tx, t))}
       |${dropLastLine(scriptWithCryptoFunctions)}
       |if rnd then pure && plix else crypto""".stripMargin
  ...
  def scriptWithV1PlixFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s""" # Plix context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && plixBalance(tx.sender) != 0
       |
       | let plix = txById && entries && balances && aFromPK && aFromStrOrRecip && height > 0
       | plix""".stripMargin
