...
import com.plixplatform.features.BlockchainFeatures._
import com.plixplatform.features._
import com.plixplatform.lagonaki.mocks.TestBlock
import com.plixplatform.lang.script.v1.ExprScript
import com.plixplatform.lang.v1.compiler.Terms.TRUE
import com.plixplatform.settings.{TestFunctionalitySettings, PlixSettings}
import com.plixplatform.state.reader.LeaseDetails
import com.plixplatform.transaction.Asset.{IssuedAsset, Plix}
import com.plixplatform.transaction.TxValidationError.AliasDoesNotExist
import com.plixplatform.transaction.assets.{IssueTransactionV1, ReissueTransactionV1}
...
import com.plixplatform.transaction.{CreateAliasTransactionV1, DataTransaction, GenesisTransaction, Transaction}
import com.plixplatform.{NoShrink, TestTime, TransactionGen, history}
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender, com.plixplatform.state.diffs.ENOUGH_AMT))
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender, com.plixplatform.state.diffs.ENOUGH_AMT))
