...
import com.plixlatform.account.{Address, KeyPair}
...
import com.plixlatform.features.BlockchainFeatures._
import com.plixlatform.features._
import com.plixlatform.lagonaki.mocks.TestBlock
import com.plixlatform.lang.script.v1.ExprScript
import com.plixlatform.lang.v1.compiler.Terms.TRUE
import com.plixlatform.settings.{TestFunctionalitySettings, PlixSettings}
import com.plixlatform.state.reader.LeaseDetails
import com.plixlatform.transaction.Asset.{IssuedAsset, Plix}
import com.plixlatform.transaction.TxValidationError.AliasDoesNotExist
import com.plixlatform.transaction.assets.{IssueTransactionV1, ReissueTransactionV1}
...
import com.plixlatform.transaction.{CreateAliasTransactionV1, DataTransaction, GenesisTransaction, Transaction}
import com.plixlatform.{NoShrink, TestTime, TransactionGen, history}
...
class RollbackSpec extends FreeSpec with Matchers with WithDomain with TransactionGen with PropertyChecks with NoShrink {
  ...
  private def transfer(sender: KeyPair, recipient: Address, amount: Long) =
    TransferTransactionV1.selfSigned(Plix, sender, recipient, amount, nextTs, Plix, 1, Array.empty[Byte]).explicitGet()
      ...
      case 2 =>
        List(
          MassTransferTransaction
            .selfSigned(Plix, sender, List(ParsedTransfer(recipient, amount), ParsedTransfer(recipient, amount)), nextTs, 10000, Array.empty[Byte])
            .explicitGet())
      case _ => List(TransferTransactionV1.selfSigned(Plix, sender, recipient, amount, nextTs, Plix, 1000, Array.empty[Byte]).explicitGet())
    }
  }
  ...
  "Rollback resets" - {
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender, com.plixlatform.state.diffs.ENOUGH_AMT))
    ...
    "plix balances" in forAll(accountGen, positiveLongGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      ...
      case (sender, initialBalance, assetAmount, recipient) =>
        withDomain() { d =>
          ...
          val issueTransaction =
            IssueTransactionV1
              .selfSigned(sender, "test".getBytes("UTF-8"), Array.empty[Byte], assetAmount, 8, reissuable = true, 1, nextTs)
              .explicitGet()
            ...
            TestBlock.create(
              ...
              Seq(
                TransferTransactionV1
                  .selfSigned(IssuedAsset(issueTransaction.id()), sender, recipient, assetAmount, nextTs, Plix, 1, Array.empty[Byte])
                  .explicitGet())
            ))
    ...
    "asset quantity and reissuability" in forAll(accountGen, positiveLongGen, byteArrayGen(10), byteArrayGen(12)) {
      case (sender, initialBalance, name, description) =>
        withDomain() { d =>
          ...
          val issueTransaction = IssueTransactionV1.selfSigned(sender, name, description, 2000, 8, reissuable = true, 1, nextTs).explicitGet()
          ...
          d.appendBlock(
            TestBlock.create(
              ...
              blockIdWithIssue,
              Seq(
                ReissueTransactionV1.selfSigned(sender, IssuedAsset(issueTransaction.id()), 2000, reissuable = false, 1, nextTs).explicitGet()
              )))
    ...
    def createSettings(preActivatedFeatures: (BlockchainFeature, Int)*): PlixSettings = {
      ...
      history.DefaultPlixSettings.copy(blockchainSettings = history.DefaultPlixSettings.blockchainSettings.copy(functionalitySettings = tfs))
    }
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender, com.plixlatform.state.diffs.ENOUGH_AMT))
