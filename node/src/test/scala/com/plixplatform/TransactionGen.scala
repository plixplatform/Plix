...
import com.plixlatform.account._
...
import com.plixlatform.lang.directives.values.V3
...
import com.plixlatform.lang.v1.testing.{ScriptGen, TypedScriptGen}
import com.plixlatform.lang.v1.{ContractLimits, FunctionHeader}
import com.plixlatform.settings.Constants
import com.plixlatform.state._
import com.plixlatform.state.diffs.ENOUGH_AMT
...
import com.plixlatform.transaction.assets.exchange._
...
import com.plixlatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, ParsedTransfer}
...
trait TransactionGenBase extends ScriptGen with TypedScriptGen with NTPTime { _: Suite =>
  ...
  protected def plix(n: Float): Long = (n * 100000000L).toLong
  ...
  val plixAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]]    = Gen.frequency((1, plixAssetGen), (10, Gen.option(bytes32gen.map(ByteStr(_)))))

  val assetPairGen: Gen[AssetPair] = assetIdGen.flatMap {
    case None => bytes32gen.map(b => AssetPair(Plix, IssuedAsset(ByteStr(b))))
    case Some(a1bytes) =>
      ...
      Gen.oneOf(Gen.const(None), a2bytesGen).map { a2 =>
        ...
        val asset2 = a2.fold[Asset](Plix)(arr => IssuedAsset(ByteStr(arr)))
  ...
  def createLease(sender: KeyPair, amount: Long, fee: Long, timestamp: Long, recipient: AddressOrAlias): Gen[LeaseTransaction with FastHashId] = {
  ...
  def createLeaseCancel(sender: KeyPair, leaseId: ByteStr, cancelFee: Long, timestamp: Long): Gen[LeaseCancelTransaction with FastHashId] = {
  ...
  val transferParamGen: Gen[(Asset, KeyPair, AddressOrAlias, Long, Long, Asset, Long, Array[Byte])] = for {
    amount     <- positiveLongGen
  ...
  def transferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransactionV1] =
    ...
    } yield TransferTransactionV1.selfSigned(Plix, sender, recipient, amount, timestamp, Plix, feeAmount, attachment).explicitGet()

  def transferGeneratorPV2(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransactionV2] =
    for {
      amount                                    <- Gen.choose(1, maxAmount)
      (_, _, _, _, _, _, feeAmount, attachment) <- transferParamGen
    } yield TransferTransactionV2.selfSigned(Plix, sender, recipient, amount, timestamp, Plix, feeAmount, attachment).explicitGet()
  ...
  def plixTransferGeneratorP(sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(sender, recipient, Plix, Plix)

  def plixTransferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(timestamp, sender, recipient, Plix, Plix)
  ...
  def createPlixTransfer(sender: KeyPair,
                          recipient: Address,
                          amount: Long,
                          fee: Long,
                          timestamp: Long): Either[ValidationError, TransferTransactionV1] =
    TransferTransactionV1.selfSigned(Plix, sender, recipient, amount, timestamp, Plix, fee, Array())

  val transferV1Gen: Gen[TransferTransactionV1] = (for {
  ...
  def versionedTransferGenP(sender: PublicKey, recipient: Address, proofs: Proofs): Gen[TransferTransactionV2] =
    (for {
      ...
      fee       <- smallFeeGen
      timestamp <- timestampGen
    } yield TransferTransactionV2.create(Plix, sender, recipient, amt, timestamp, Plix, fee, Array.emptyByteArray, proofs).explicitGet())
      .label("VersionedTransferTransactionP")

  val transferWithPlixFeeGen: Gen[TransferTransactionV1] = for {
    (assetId, sender, recipient, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransactionV1.selfSigned(assetId, sender, recipient, amount, timestamp, Plix, feeAmount, attachment).explicitGet()

  val selfTransferWithPlixFeeGen: Gen[TransferTransactionV1] = for {
    (assetId, sender, _, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransactionV1.selfSigned(assetId, sender, sender, amount, timestamp, Plix, feeAmount, attachment).explicitGet()
  ...
  val issueParamGen: Gen[(KeyPair, Array[Byte], Array[Byte], Long, Byte, Boolean, Int, Long)] = for {
    sender      <- accountGen
  ...
  val invokeScriptGen: Gen[InvokeScriptTransaction] = for {
    sender      <- accountGen
    ...
    fee       <- smallFeeGen
    timestamp <- timestampGen
  } yield InvokeScriptTransaction.selfSigned(sender, dappAddress.toAddress, Some(fc), po.toSeq, fee, Plix, timestamp).explicitGet()
  ...
  val orderParamGen: Gen[(KeyPair, KeyPair, AssetPair, OrderType, Long, Long, Long, Long, Long)] = for {
    sender     <- accountGen
    matcher    <- accountGen
    pair       <- assetPairGen
  ...
  val exchangeTransactionGen: Gen[ExchangeTransaction] = for {
    ...
    buyerMatcherFeeAssetId  <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, buyerAnotherAsset, Plix)
    sellerMatcherFeeAssetId <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, sellerAnotherAsset, Plix)
    r <- Gen.oneOf(
  ...
  def exchangeV2GeneratorP(buyer: KeyPair,
                           ...
                           buyMatcherFeeAssetId: Asset = Plix,
                           sellMatcherFeeAssetId: Asset = Plix,
