...
import com.plixlatform._
import com.plixlatform.account.{Address, KeyPair, PublicKey}
...
import com.plixlatform.db.{WithDomain, openDB}
import com.plixlatform.features.BlockchainFeatures
import com.plixlatform.history.{StorageFactory, randomSig}
...
import com.plixlatform.lang.v1.compiler.{CompilerContext, ExpressionCompiler}
import com.plixlatform.mining._
...
import com.plixlatform.transaction.TxValidationError.SenderIsBlacklisted
...
import com.plixlatform.transaction.{Asset, Transaction, _}
...
class UtxPoolSpecification
  ...
  private def mkBlockchain(senderAccount: Address, senderBalance: Long) = {
    ...
    val origSettings    = PlixSettings.fromRootConfig(config)
  ...
  private def transfer(sender: KeyPair, maxAmount: Long, time: Time) =
    ...
    } yield TransferTransactionV1.selfSigned(Plix, sender, recipient, amount, time.getTimestamp(), Plix, fee, Array.empty[Byte]).explicitGet())
  ...
  private def massTransferWithRecipients(sender: KeyPair, recipients: List[PublicKey], maxAmount: Long, time: Time) = {
    ...
    val txs = for { fee <- chooseNum(minFee, amount) } yield
      MassTransferTransaction.selfSigned(Plix, sender, transfers, time.getTimestamp(), fee, Array.empty[Byte]).explicitGet()
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createPlixTransfer(sender, recipient, amountPart, fee, time.getTimestamp()).explicitGet()
  ...
  private def transactionV1Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransactionV1] = accountGen.map { recipient =>
    TransferTransactionV1.selfSigned(Plix, sender, recipient, plix(1), ts, Plix, feeAmount, Array.emptyByteArray).explicitGet()
  }

  private def transactionV2Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransactionV2] = accountGen.map { recipient =>
    TransferTransactionV2.selfSigned(Plix, sender, recipient, plix(1), ts, Plix, feeAmount, Array.emptyByteArray).explicitGet()
  }
  ...
  "UTX Pool" - {
    ...
    "correctly process constrainst in packUnconfirmed" in {
      withDomain(plixlatform.history.TransfersV2ActivatedAt0PlixSettings) { d =>
