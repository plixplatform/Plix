package com.plixlatform.lagonaki.unit

import com.plixlatform.account.PublicKey
import com.plixlatform.block.{Block, SignerData}
...
class BlockSpecification extends PropSpec with PropertyChecks with TransactionGen with Matchers with NoShrink {
  ...
  val blockGen = for {
    ...
    paymentTransaction        <- plixTransferGeneratorP(time, sender, recipient)
    transferTrancation        <- transferGeneratorP(1 + time, sender, recipient, assetId, Plix)
    anotherPaymentTransaction <- plixTransferGeneratorP(2 + time, sender, recipient)
  ...
  def bigBlockGen(amt: Int): Gen[Block] =
    for {
      ...
      sender                                    <- accountGen
      recipient                                 <- accountGen
      paymentTransaction: TransferTransactionV1 <- plixTransferGeneratorP(time, sender, recipient)
    } yield
      Block
        .buildAndSign(3,
                      time,
