...
import com.plixlatform.api.http.{InvokeScriptRequest, SignedInvokeScriptRequest}
import com.plixlatform.common.state.ByteStr
import com.plixlatform.common.utils.{Base64, _}
import com.plixlatform.lang.v1.compiler.Terms
import com.plixlatform.lang.v1.compiler.Terms.{ARR, CONST_LONG, CaseObj}
import com.plixlatform.lang.v1.compiler.Types.CASETYPEREF
import com.plixlatform.lang.v1.{ContractLimits, FunctionHeader}
import com.plixlatform.transaction.Asset.{IssuedAsset, Plix}
import com.plixlatform.transaction.TxValidationError.NonPositiveAmount
...
class InvokeScriptTransactionSpecification extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("Signed InvokeScriptTransactionRequest parser") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(1, Plix))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
  ...
  property("can't have zero amount") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(0, Plix))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs = List("CC1jQ4qkuVfMvB2Kpg2Go6QKXJxUFC8UUswUxBsxwisrR8N5s3Yc8zA6dhjTwfWKfdouSTAnRXCxTXb3T6pJq3T")
    )
    req.toTx shouldBe Left(NonPositiveAmount(0, "Plix"))
    AddressScheme.current = DefaultAddressScheme
  }
  ...
  property("can't have negative amount") {
    ...
    req.toTx shouldBe Left(NonPositiveAmount(-1, "Plix"))
    AddressScheme.current = DefaultAddressScheme
  }
}
