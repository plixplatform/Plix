...
abstract class GenericTransactionSpecification[T <: com.plixlatform.transaction.Transaction]
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen {

  def transactionParser: com.plixlatform.transaction.TransactionParserFor[T]
  def updateProofs(tx: T, p: Proofs): T
  def generator: Gen[((Seq[com.plixlatform.transaction.Transaction], T))]
