...
import com.plixlatform.transaction.assets.exchange.{AssetPair, Order, OrderType, _}
import com.plixlatform.transaction.smart.Verifier
  ...
  property("AssetPair test") {
    forAll(assetIdGen, assetIdGen) { (assetA, assetB) =>
      whenever(assetA != assetB) {
        val assetAId: Asset = assetA.fold[Asset](Plix)(arr => IssuedAsset(arr))
        val assetBId: Asset = assetB.fold[Asset](Plix)(arr => IssuedAsset(arr))
