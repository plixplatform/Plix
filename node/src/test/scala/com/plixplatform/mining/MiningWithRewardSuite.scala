package com.plixlatform.mining
...
import com.plixlatform.database.{Keys, LevelDBWriter}
...
import com.plixlatform.transaction.{Asset, BlockchainUpdater, GenesisTransaction, Transaction}
import com.plixlatform.utx.UtxPoolImpl
import com.plixlatform.wallet.Wallet
import com.plixlatform.{TransactionGen, WithDB}
    ...
    val bps: Seq[BlockProducer] = Seq(
      (ts, reference, account) => {
        ...
        val tx1 = TransferTransactionV2
          .selfSigned(Plix, account, recipient1, 10 * Constants.UnitsInWave, ts, Plix, 400000, Array())
          .explicitGet()
        val tx2 = TransferTransactionV2
          .selfSigned(Plix, account, recipient2, 5 * Constants.UnitsInWave, ts, Plix, 400000, Array())
          .explicitGet()
...
object MiningWithRewardSuite {
  ...
  val commonSettings: PlixSettings                    = PlixSettings.fromRootConfig(loadConfig(ConfigFactory.load()))
