...
inTask(assembly)(
  Seq(
    test := {},
    assemblyJarName := s"plix-all-${version.value}.jar",
...
// Logback creates a "plix.directory_UNDEFINED" without this option.
bashScriptExtraDefines ++= Seq(
  s"""addJava "-Dplix.defaults.directory=/var/lib/${(Universal / normalizedName).value}"""",
  s"""addJava "-Dplix.defaults.config.directory=/etc/${(Universal / normalizedName).value}""""
)

inConfig(Universal)(
  Seq(
    mappings += (baseDirectory.value / s"plix-${network.value}.conf" -> "doc/plix.conf.sample"),
    mappings := {
...
inConfig(Linux)(
  Seq(
    maintainer := "plixlatform.com",
    packageSummary := "Plix node",
    packageDescription := "Plix node"
  ))
...
moduleName := s"plix${network.value.packageSuffix}" // plix-*.jar instead of node-*.jar
executableScriptName := moduleName.value // bin/plix instead of bin/node
